...
methods that combine predictors, we propose to combine active learners. In
addition to combining active learners using bandits (which have been partially
investigated before), we propose to combine active learning recommendations
using rank aggregation approaches. Using benchmark datasets, we demonstrate
that there is no need to pick a particular active learner a priori, since we
can cheaply combine them. We discuss challenges to finding good rewards for
bandit approaches, and show that rank aggregation performs well. Our results
show that for large datasets, a combination of active learners outperforms
passive learning.
...
estimates for each test example. For example, in logistic regression with only
...
\begin{IEEEeqnarray}{lCl}
	h(\bm{x}; \bm{\theta}) &=& p(y=1 \mid \bm{x}; \bm{\theta})
...
that we could exploit the current set of labelled data to identify more
informative unlabelled examples to annotate. For instance we can pick examples
near the decision boundary of the classifier (with uncertain class probability
estimates), where we are still unsure which class the example belongs to.
...
active learning heuristics. In supervised learning, combining predictors is a
well-studied problem. Many techniques such as AdaBoost~\citep{freund96} (which
averages predictions from a set of models) and decision trees~\citep{breiman84}
(which select one model for making predictions in each region of the input
space) have been shown to perform better than just using a single model.
Inspired by this success, we propose to combine active learning suggestions
with bandit and social choice theories in Section~\ref{sec:expert}. The use of
bandit algorithms, where we only pick one heuristic at each time step, has been
...
random sample at the beginning. These become the labeled \emph{Training} set
$\Labeled_T \subset \X \times \Y$, and the rest form the \emph{Unlabeled} set
$\Unlabeled \subset \X$.
...
choosing the next example. This motivates us to come up with a rule $s(\bm{x};
h)$ that gives each unlabeled example a score based only on their feature
vector $\bm{x}$ and the current classifier $h$. Recall that the classifier
produces $p(\Y)$, a probability estimate for each class. We use these
probability estimates from the classifier over the unlabeled examples to
calculate the scores:
\begin{IEEEeqnarray}{lCl}
	s : p(\Y)^{U} \rightarrow \mathbb{R}^{U}
\end{IEEEeqnarray}
where $U$ is the size of the unlabeled set $\Unlabeled$. The value of
$s(\bm{x}; h)$ indicates the informativeness of example $\bm{x}$, and we would
like to choose the example with the largest value of $s(\bm{x}; h)$. This
will be our active learning rule $r$:
\begin{IEEEeqnarray}{lCl}
	r(\bm{x}; h) &=& \argmax_{x \in \Unlabeled} s(\bm{x}; h)
\end{IEEEeqnarray}
...
\begin{IEEEeqnarray}{lCl}
	r_{LC}(\bm{x}; h) &=& \argmax_{x \in \Unlabeled}
	\left\{ - \max_{y \in \Y} p(y | \bm{x}; h) \right\}
\end{IEEEeqnarray}
where, for consistency, we have flipped the sign of the score function so that
...
\begin{IEEEeqnarray}{lCl}
	r_{SM}(\bm{x}; h) &=& \argmax_{x \in \Unlabeled}
	\left\{ - \left ( \max_{y \in \Y} p(y | \bm{x}; h) -
	\max_{z \in \Y \setminus \{y\}} p(z | \bm{x}; h) \right) \right\}
\end{IEEEeqnarray}
where we again flip the sign of the score function. Since the sum of all
probabilities must be 1, the smaller the margin is, the more uncertain we are
about the object's class membership.
...
\begin{IEEEeqnarray}{lCl}
	r_{QBBM}(\bm{x}; h) &=& \argmax_{x \in \Unlabeled}
...
\end{IEEEeqnarray}
where $D_{\mathrm{KL}}(p_b\|p_\B)$ is the KL divergence from $p_B$ (the
...
\begin{table}[t]
	\caption {Summary of active learning heuristics used in our experiments.
			  ...
			  average prediction distribution of $\B$. For consistency,
	...
	\begin{tabular}{lll}
		...
		Highest Entropy &
			...
			\\[2ex]
		Smallest Margin &
			$\argmax_{x \in \Unlabeled} \left\{ - \left( \max_{y \in \Y} p(y | \bm{x}; h) -
            \max_{z \in \Y \setminus \{y\}} p(z | \bm{x}; h) \right) \right\}$
			\\[2ex]
		Smallest QBB Margin &
			$\argmax_{x \in \Unlabeled} \left\{ - \left( \max_{y \in \Y} p(y | \bm{x}; \B) -
            \max_{z \in \Y \setminus \{y\}} p(z | \bm{x}; \B) \right) \right\}$
			\\[2ex]
		Largest QBB KL &
...
choice theory). Refer to Fig.~\ref{fig:pipeline} for an overview of the
...
\begin{figure}[htbp]
	...
	\begin{subfigure}[t]{\textwidth}
		...
		predicted class probabilities on the unlabeled data.}
    ...
    \begin{subfigure}[t]{\textwidth}
		...
		\end{tikzpicture}
		\caption{In the active learning pipelines with rank aggregation
		...
		\label{fig:pipeline-bandit}
	\end{subfigure}%
	\caption[Active learning pipeline]{Active learning pipelines}
	\label{fig:pipeline}
\end{figure}
...
What would be an appropriate reward $w$ in this setting? One option is the
...
labels of objects in the test set. Let $w$ be the reward observed. We now
...
\begin{algorithm}[htbp]
	...
	\caption{Thompson sampling with normally distributed rewards. Notations:
			...
			average reward, and $\tau^2$ is the known variance parameter of
...
that maximizes the following upper bound:
    ...
    \end{IEEEeqnarray}
where $\overline{w_i}$ is the average of the rewards from $r_i$ that we
...
\begin{algorithm}[htbp]
	...
	\Fn{\Select()}{
		...
		\For{$i \in \{1, 2, ..., R\}$}{
			...
			$W_i = \dfrac{e^{-\beta * L_i}}{\sum_j e^{-\beta * L_j}}$ \;
			}
...
labeling 10 random samples and using them as the initial training set. Note
