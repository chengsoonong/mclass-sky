...
addition to combining active learners using bandits (which has been partially
...
indicate that for large datasets, a combination of active learners outperforms
passive learning.
...
class the example belongs to (i.e. the class probability estimates are uncertain).
...
simply set to 1000. To speed up the training time while still using the
Gaussian kernel, we approximate the feature map of the kernel with Random
Kitchen Sinks \citep{rahimi08}, transforming the raw features $\bm{x}_i$ into a
fixed 100-dimensional feature vector $f(\bm{x}_i)$. In the multiclass case, we
use the One-vs-Rest strategy, where for every class, we build a binary
