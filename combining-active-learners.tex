...
$\Normal(\mu_r, \sigma_r^2)$ for each $i$ and select heuristic $r_*$ that has
...
upper bound can be high when the expected reward is high or when the
...
methods. In Optimally Confident UCB (OC-UCB), \cite{lattimore15} suggested that
we pick the heuristic that maximizes the following upper bound:
    ...
    \end{IEEEeqnarray}
where $\overline{w_r}$ is the average of the rewards from $r_r$ that we observe
...
\begin{algorithm}[htbp]
	...
	\caption{Optimally Confident UCB. Notations: $\R$ is the set of heuristics,
			 ...
			 current time step, $T_r(t)$ counts of how many times heuristic $i$
			 has been selected, $w$ is the reward received, and
			 $\overline{w}$ is the average of the rewards from $r$ so
			 far.}
	\label{alg:oc-ucb}
\end{algorithm}
...
\begin{algorithm}[htbp]
	...
	current time step, $T_r(t)$ counts of how many times heuristic $i$ has been
...
\cite{auer2002nonstochastic} proposed an extension, EXP4, that now includes the
...
suggestions \cite{baram04, hsu15}.
...
labels. In particular, the unlabeled pool size is 70\% of data up to a maximum
of 10,000 examples. The test pool consists of the remaining examples up to a
maximum of 20,000---we assume labels are available here. We use this test pool
to plot the learning curves and, in the case of bandit algorithms, to compute
the rewards.
...
selected candidates from the unlabeled pool into the algorithm. We use
Scikit-learn implemetnation of logistic regression with an L2 loss
