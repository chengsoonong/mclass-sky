...
Getting spectroscopic labels is expensive. So far, when astronomers pick which objects to obtain
spectra from, not much attention is paid to whether the labelling information can improve
the performance of the classifiers. To illustrate this, imagine there is a group of objects
...
In the active learning setting, the feature space $\X$ is split into two parts. The
set $\Labelled \subset \X \times \Y$ contains all the examples that have been labelled
by an expert. All the remaining unlabelled examples form $\Unlabelled \subset \X$.
The question now is how to select the next training example from $\Unlabelled$. In practical terms, 
where should we point the telescope to next to obtain a spectrum? To answer this question,
we need a rule $r(\bm{x})$ that can assign score to each object, based solely on their features. The
score should reflect the amount of new information we would gain if we were to label the object.
We will discuss various heuristics in the next section. For now assuming that we have $r(\bm)$,
...
\begin{algorithm}[tbp]
	\caption{The general active learning algorithm}
	\label{alg:active}
	\begin{algorithmic}[1]
