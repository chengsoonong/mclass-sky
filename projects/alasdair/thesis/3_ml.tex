...
\begin{algorithm}[h]
	...
	\label{alg:active}
	\begin{algorithmic}[1]
		\Procedure {ActiveLearner}{$\Unlabelled$, $\Labelled_T$, $h$, $r$, $n$, $t$}
			\While {$|\Labelled_T| < n$}
...
and the probably density function (PDF) of the reward is
    \begin{IEEEeqnarray}{lCl}
        f(w_i \mid \nu_i)
        &=& \frac{1}{\tau_i \sqrt{2\pi}} \exp\left[ -\frac{(w_i - \nu_i)^2}{2 \tau_i^2} \right]
         \label{eqn:rlike}
    \end{IEEEeqnarray}
...
Thus the PDF of $\nu_i$ is
    \begin{IEEEeqnarray}{lCl}
        f(\nu_i)
        &=& \frac{1}{\sigma_i \sqrt{2\pi}} \exp\left[ -\frac{(w_i - \mu_i)^2}{2 \sigma_i^2} \right]
         \label{eqn:rprior}
    \end{IEEEeqnarray}
...
\begin{algorithm}[h]
	...
	\label{alg:bandit}
	\begin{algorithmic}[1]
		\Procedure {ActiveBandit}{$\Unlabelled$, $\Labelled_T$, $h$, $n$, $t$,
...
As we shall see in Chapter \ref{cha:expt2}, the reward function dynamically evolves as the training
...
have been made in the literature. For example \citeN{gupta11} introduce the Dynamic Thompson
