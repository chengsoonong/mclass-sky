...
\section{Interior-point methods} 
    ...
    \subsection{Feasbility and the basic phase I method}
        ...
        \begin{equation}\label{e:phase_i_impl_ineq_constrd_cvx_opt_problem}
            \begin{aligned}
            & {\text{minimize}} && f_0(x) + \sum_{i=1}^{m}{I_{-}(f_i(x)-s)} \\
            & \text{subject to} && Ax = b,
            \end{aligned}
        \end{equation}
        ...
        \begin{equation}\label{e:phase_i_ineq_constrd_cvx_opt_problem_approx_equiv}
            \begin{aligned}
            & {\text{minimize}} && 
            f_0(x) + \sum_{i=1}^{m}{-\left(\frac{1}{t}\right)\log(s-f_i(x))}  \\
            & \text{subject to} && Ax = b,
            \end{aligned}
        \end{equation}
        or equivalently
        ...
        \label{a:basic_conceptual_cp_alg}\mbox{}\\
            \ind \textbf{given} strictly feasible $z = (x, s) \in \mathbb{R}^n \times \mathbb{R}$, $t \coloneqq t^{(0)} > 0$, $\mu > 1$, tolerance $\epsilon > 0$ \\
            \ind $k \coloneqq 0$. \\
            \ind \textbf{repeat} \\
            \ind\ind 1. \emph{Centering step.} \\
