...
\section{Cutting plane methods}
    ...
    \subsection{Analytic center cutting-plane method}
        In this section we discuss the analytic center cutting-plane method. The analytic center cutting-plane method (ACCPM) generates the query point at each iteration in the following way. Suppose at iteration $k$ we have
        \begin{equation*}
            \mathcal{P}_k = \{z \;|\; c_i^\transpose z \leq d_i, i = 1, \dots, m_0, a_i^\transpose z \leq b_i, i = 1, \dots, m_k\}
        \end{equation*}
        where the constraints $c_i^\transpose z \leq d_i$ are those that specify $\mathcal{P}_0$ and the constraints $a_i^\transpose z \leq b_i$ are the $m_k$ cutting planes from the previous $k$ iterations. (We can make the choice to return multiple cutting plans each iteration.) We choose the query point for the $k+1$-th iteration to be the \emph{analytic center} of $\mathcal{P}_k$, which is the point $x \in \interior \mathcal{P}_k$ that solves the minimization problem
        \begin{equation*}
            \begin{aligned}
            & {\text{minimize}} && -\sum_{i=1}^{m_0}\log(d_i - c_i^\transpose x) - \sum_{i=1}^{m_k} \log(b_i - a_i^\transpose x).
            \end{aligned}
        \end{equation*}

        To summarize this discussion we have the following:
