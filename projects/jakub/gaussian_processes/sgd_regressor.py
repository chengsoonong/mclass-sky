...
def main(path):
    ...
    if True:
        X_pred = np.empty((PLOT_SAMPLES, INPUT_DIM))
        X_pred[:, :4] = np.mean(X[:, :4], axis=0)
        X_pred[:, 4] = np.linspace(np.min(X[:, 4]), np.max(X[:, 4]), num=PLOT_SAMPLES)

        y_pred = sgd.predict(X_pred)
        plt.plot(X[:, 4], y, "ro", markersize=0.5)
        plt.errorbar(X_pred[:, 4], y_pred, capsize=0)

        plt.show()

    print("Score: {}".format(sgd.score(test_X, test_y)))
