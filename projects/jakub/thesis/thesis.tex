...
Regression is the task of reconstructing a function $f$ using a finite number of examples of its inputs and outputs.

Let $\cX \subset \bbR^d$ be the bounded space of inputs of $f$. Then $f$ maps $\cX$ to $\bbR$.

We are given a set of $n$ noisy samples\[
  \cD = \left\{\left(\bx_1, y_1\right), \dots, \left(\bx_n, y_n\right)\right\} \text{,}
\] where for each $i = 1, \dots, n$, \[
  y_i = f(\bx_i) + \epsilon \text{ for some error } \epsilon \sim \cP\text{,}
\] where $\cP : \bbR \to \bbR_{\geq0}$ is the probability distribution of the noise.

We want to find a function $\hat f$ that reconstructs $f$ using the limited information that we have about it. The likelihood of each possible explanation $\hat f_c : \bbR^d \to \bbR$ being correct is \[
    p\left(\hat f_c \;\middle|\; \cD \right) \text{.}
\] We wish to find the $\hat f_c$ that maximises it:\[
    \hat f \coloneqq \argmax_{\hat f_c} p\left(\hat f_c \;\middle|\; \cD \right) \text{.}
\]

The formalisation of this likelihood is dependent on our assumptions. For example, it will depend on our assumptions about $\cP$ and about $f$. The space we search over will depend on our assumptions about the form that $\hat f$ is in.

In our specific case we will always assume that \[
    \cP = \cN(0, \sigma^2)
\] for some variance $\sigma^2$ that we leave as a hyperparameter: a parameter that depends on the distribution of our data.

We want to apply this technique to the problem of finding the redshift of an object from its photometric measurements, as a cheaper means of finding objects' redshifts than spectroscopy. Given a set of example objects' photometric measurements and spectoscopic redshifts, we wish to predict the spectroscopic redshifts for any new object whose photometric colours have been measured. Hence, $\cX \subset \bbR^5$ is the space of photometric measurements.
...
\jn{TODO: make very helpful plots}
