...
\citep{reyes18} to combine uncertainty information from multiple labels.
However, as far as we know, other aggregations methods have not been explored
and this is the first time that social choice theory is used to rank and
...
\begin{enumerate}
	...
	running them on 11 benchmark datasets from the UCI Machine Learning
	...
	\item For evaluation, we propose two metrics: the mean posterior balanced
	accuracy (MPBA) and the strength of an algorithm. The MPBA extends the the
	metric proposed in \cite{brodersen10} and is an accuracy measure that takes
	class imbalance into account. The strength measure builds on the work of
	\cite{baram04} and allows us to summarize the performance of an active
...
well as the more simpler ones \citep{tran15}. Thus we do not consider them in
this paper.

A more comprehensive treatment of these active
learning heuristics can be found in \cite{settles12}.
...
\begin{align}
	b : ({J_\R} \times [0, 1])^{T} \rightarrow J_\R
\end{align}
...
\cite{cappe13} suggest that we can instead consider the KL-divergence between
the distribution of the current estimated reward and that of the upper bound.
...
that provides a ranking of the unlabeled examples according the their scores.
...
These datasets were chosen such that we have an equal number of binary
...
labeling 10 random instances and using them as the initial training set. The
heuristics are fast enough such that we can assign a score to every unlabeled
instance at every time step. We use logistic regression with a Gaussian kernel
approximation and an L2 regularizer. In the binary case, the loss function is
\begin{align}
...
is not too large, and $C$ is another regularization hyperparameter which we use
grid search to find the best value between $10^{-6}$ and $10^6$. To speed up
...
heuristics with information density weighting to the bandits would impact
the performance.
...
\ref{fig:selection-thompson-params-ionosphere}, \textsc{passive}
...
\citep{xu07} or by maximizing the diversity in a batch \citep{brinker03}. How
...
\begin{figure}[tbp]
	...
	10 trials. The dashed blue line shows the performance obtained from
...
\begin{figure}[tbp]
	...
	curves are averaged over 10 trials. The dashed blue line shows the
...
\begin{figure}[tbp]
	...
	selection]{The effect of the the initial values of the parameters in
	...
	(the purple line) and \textsc{qbb-kl} (the light green line) tend to get
...
\begin{figure}[tbp]
	...
	\caption[Effect of the pool size]{Effect of the
