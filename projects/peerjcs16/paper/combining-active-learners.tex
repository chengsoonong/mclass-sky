...
computationally inefficient. A more comprehensive treatment of these active
...
unnormalized scores. \cite{hsu15} studied an improved version of EXP4, called
EXP4.P, and used importance weighting to estimate the true classifier
...
\cite{cappe13} suggest that we can instead consider the KL-divergence from the
current estimated reward distribution. In the case of normally distributed
rewards with known variance $\sigma^2$, the chosen heuristic would be
    \begin{align}
		r_* = \argmax_{i} \overline{w_i} +
...
Table~\ref{tab:rank} for an example.
...
curves of all the methods.}, and $m(h, t)$ is the performance achieved using
...
In this section, we explore how changing the hyperparameters might affect the
results. We pick one binary dataset (glass) and one multiclass dataset
(ionosphere) to investigate. Both of these are small enough to allow us
to iterate through many hyperparameter values quickly, and as shown in
Figure \ref{fig:learning_curves-small}, active learning does improve
over passive learning here.

\subsubsection{Information Density Parameter}

The heuristics with information density weighting has one tunable
hyperparameter--the $\gamma$ in the Gaussian kernel (see Equation
\ref{eqn:kernel}). We examine five different values for $\gamma$: the
50th, 60th, 70th,
...
\begin{figure}[tbp]
	...
	\textsc{w-confidence} and \textsc{w-margin}: We test 6 gamma values}
	\label{fig:selection}
\end{figure}
...
\begin{figure}[tbp]
	...
	\caption[asdf]{Effect of the prior parameters on Thompson sampling}
	\label{fig:selection}
\end{figure}
...
\begin{figure}[tbp]
	...
	\begin{subfigure}[t]{\textwidth}
		...
		\label{fig:selection-thompson-params-glass}
    \end{subfigure}
	\caption[Effect of hyperparameters on Thompson sampling]{The effect of the
	the prior mean and variance parameters on \textsc{thompson} using two
	datasets--glass and ionosphere. The plots show how often each of the
	heuristics gets selected over time. The selection frequencies are averaged
	over 10 trials.}
	\label{fig:selection}
\end{figure}
