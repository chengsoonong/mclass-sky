...
%DIF ADD combining-active-learners.tex      Mon Mar 26 16:35:42 2018
...
\DIFdelbegin \DIFdel{To compare the algorithms, we run }\DIFdelend \DIFaddbegin \DIFadd{This paper makes the following two main contributions:
	...
	accuracy (MPBA) and the strength of an algorithm. The MPBA extends the the
	metric proposed in \mbox{%DIFAUXCMD
\cite{brodersen10} }\hspace{0pt}%DIFAUXCMD
and is an accuracy measure that takes
	class imbalance into account. The strength measure builds on the work of
	\mbox{%DIFAUXCMD
\cite{baram04} }\hspace{0pt}%DIFAUXCMD
and allows us to summarize the performance of an active
	learner or combiner, relative to passive learning, in one number.
}\end{enumerate}
\DIFaddend 
