...
%DIF ADD combining-active-learners-v2.tex   Wed Apr  4 15:35:19 2018
...
before~\citep{baram04, hsu15}. \DIFaddbegin \DIFadd{Borda count, a simple rank aggregation method,
...
improve the performance of multiple tasks (e.g. part-of-speech tagging and name
entity recognition). Borda count has also been used in multi-label learning
\mbox{%DIFAUXCMD
\citep{reyes18} }\hspace{0pt}%DIFAUXCMD
to combine uncertainty information from multiple labels.
}\DIFaddend However, as far as we know, \DIFaddbegin \DIFadd{other aggregations methods have not been explored
and }\DIFaddend this is the first time that social choice theory is used to rank and
aggregate suggestions \DIFaddbegin \DIFadd{from multiple active learners}\DIFaddend .

\DIFdelbegin \DIFdel{To compare the algorithms, we run }\DIFdelend \DIFaddbegin \DIFadd{This paper makes the following two main contributions:
}\begin{enumerate}
	\item \DIFadd{We empirically compare 4 bandit and 3 rank aggregation algorithms in
	the context of combining active learning heuristics. This is done by
	running }\DIFaddend them on 11 benchmark datasets from the UCI Machine Learning
	...
	described in Section~\ref{sec:expt} and~\ref{sec:discuss}.
	\DIFaddbegin \item \DIFadd{For evaluation, we propose two metrics: the mean posterior balanced
...
that allows us to summarize the performance of an active
	learner or combiner, relative to passive learning, with one number.
	Compared to \mbox{%DIFAUXCMD
\cite{baram04}}\hspace{0pt}%DIFAUXCMD
, our strength measure assigns a higher number
	for better active learning methods and ensures that it is upper-bounded by
	1 for easier comparison across datasets.
}\end{enumerate}
\DIFaddend 
...
computationally inefficient. For example, in the variance minimization
heuristic, in order to compute the score of a candidate example, we first need
to give it each of the possible labels, put it in the training set, and update
the classifier to compute the reduction in the model variance. This is
expensive to run since in each iteration, the classifier needs to be retrained
$k \times U$ times, where $k$ is the number of classes and $U$ is the size of
the unlabeled pool. There are techniques to speed this up, such as using online
training or assigning a score to only a small subset of the unlabeled pool.
Some preliminary experiments showed that these heuristics do not perform as
well as the more simpler ones \mbox{%DIFAUXCMD
\citep{tran15}}\hspace{0pt}%DIFAUXCMD
. Thus we do not }\DIFaddend consider them in
this paper\DIFdelbegin \DIFdel{as they are
computationally inefficient}\DIFdelend .

\DIFaddbegin \DIFadd{A more comprehensive treatment of these active
learning heuristics can be found in \mbox{%DIFAUXCMD
...
\begin{align}
	b : \DIFaddbegin \DIFadd{(}{\DIFadd{J_\R}} \DIFadd{\times }\DIFaddend [0, 1]\DIFdelbegin \DIFdel{^{R \times T} }\DIFdelend \DIFaddbegin \DIFadd{)^{T} }\DIFaddend \rightarrow J_\R
\end{align}
...
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \begin{tikzpicture}[auto, node distance=2cm,>=latex', scale=0.8, transform shape]
		\node [input, name=test input] {};
		\node [block, right of=test input, node distance=3cm] (classifier) {Train with classifier $h$};
		\node [block, right of=classifier, node distance=5cm] (heuristic) {Assign scores with $s$};
		\node [block, right of=heuristic, node distance=5cm] (best) {Select candidate \\ with highest score};
	...
	\end{tikzpicture}
	\DIFaddendFL \caption{In the active learning pipeline with bandit algorithms, we
...
\cite{cappe13} suggest that we \DIFaddbegin \DIFadd{can }\DIFaddend instead consider the KL-divergence \DIFdelbegin \DIFdel{when finding
the }\DIFdelend \DIFaddbegin \DIFadd{between
the distribution of the current estimated reward and that of the }\DIFaddend upper bound.
In the case of normally distributed rewards with known variance $\sigma^2$, the
chosen heuristic would be
    \begin{align}
		r_* = \argmax_{i} \overline{w_i} +
...
$\Unlabeled$. }\DIFdelend \DIFaddbegin \DIFadd{examples according the their scores.
...
criterion, i.e. the winner chosen by the algorithm is always preferred when
...
\DIFaddbegin \DIFadd{These datasets were chosen such that we have an equal number of binary
and multiclass datasets, and a mixture of small and large datasets.
}\DIFaddend 
...
labeling 10 random instances and using them as the initial training set. \DIFdelbegin \DIFdel{Note
that in practice, there can be a large amount of unlabeled data, so for
computational reasons, we only }\DIFdelend \DIFaddbegin \DIFadd{The
heuristics are fast enough such that we can }\DIFaddend assign a score to \DIFdelbegin \DIFdel{a small random sample from the
unlabeled
pool. In particular, we feed 300 randomly selected candidates from
the unlabeled pool into the algorithm. }\DIFdelend \DIFaddbegin \DIFadd{every unlabeled
instance at every time step. }\DIFaddend We use logistic regression with a Gaussian kernel
approximation and an L2 \DIFdelbegin \DIFdel{loss}\DIFdelend \DIFaddbegin \DIFadd{regularizer}\DIFaddend . In the binary case, the loss function is
\begin{align}
...
is not too large, and $C$ is another regularization hyperparameter which we \DIFdelbegin \DIFdel{simply set to 1000. }\DIFdelend \DIFaddbegin \DIFadd{use
grid search to find the best value between $10^{-6}$ and $10^6$. }\DIFaddend To speed up
the training time while using the Gaussian kernel, we approximate the feature
map of the kernel with Random Kitchen Sinks \citep{rahimi08}, transforming the
raw features $\bm{x}_i$ into a fixed 100-dimensional feature vector
$f(\bm{x}_i)$. In the multiclass case, we use the One-vs-Rest strategy, where
for every class, we build a binary classifier that determines whether a
particular example belongs to that class or not. For the QBB algorithms, we
train a committee of 7 classifiers, where each member is given a sample of 100
examples that have already been labeled.
...
\DIFaddbegin \begin{figure}[htbp]
	...
	\caption[Strength Measure]{\DIFaddFL{An illustration of the strength measure. It is
...
%DIFDELCMD < 	\centering
%DIFDELCMD < 	\includegraphics[width=0.7\textwidth]{figures/strength}
%DIFDELCMD < 	%%%
%DIFDELCMD < \caption[Strength Measure]{%
{%DIFAUXCMD
\DIFdelFL{An illustration of the strength measure. It is
	proportional to the shaded area between the (red) passive learning curve
	and the (blue) active learning curve. The bigger the area is, the more the
	active learner outperforms the passive learner. The top dotted line
	...
	%DIFAUXCMD
%DIFDELCMD < \label{fig:strength}
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{The heuristics with information density weighting and Thompson sampling have a
...
iterate through many hyperparameter values quickly. With }\textsc{\DIFadd{w-confidence}}\DIFadd{,
}\textsc{\DIFadd{w-margin}}\DIFadd{, and }\textsc{\DIFadd{w-entropy}}\DIFadd{, we set $\gamma$ in the Gaussian
...
\ref{fig:gamma}. For }\textsc{\DIFadd{thompson}}\DIFadd{, the prior values for $\mu$, $\sigma^2$
...
\ref{fig:learning_curves-mpba-large}).
}\DIFaddend 
...
\ref{fig:selection-thompson-params-ionosphere}, }\textsc{\DIFadd{passive}}
...
batch-mode active learning is preferred, where it is much more cost efficient
to obtain data from multiple objects at the same time to be labeled. One naive
extension is to simply choose the $m$ highest ranked objects using our current
methods. However, it is possible to have two unlabeled objects whose class
membership we are currently uncertain about, but because they have very similar
feature vectors, labeling only one of them would allow us to predict the label
of the other one easily. More sophisticated batch-mode active learning
heuristics have been proposed, for example by using cluster analysis
\mbox{%DIFAUXCMD
\citep{xu07} }\hspace{0pt}%DIFAUXCMD
or by maximizing the diversity in a batch \mbox{%DIFAUXCMD
\citep{brinker03}}\hspace{0pt}%DIFAUXCMD
. How
to aggregate suggestions from these heuristics is an interesting problem for
future work.
}
...
\DIFaddend \begin{figure}[tbp]
	...
	recall and precision, gives an equal representation to each class. \DIFaddbeginFL \DIFaddFL{The
	boxes represent the quartiles and the whiskers extend to 1.5 times of the
	interquartile range.}\DIFaddendFL }
...
\begin{figure}[tbp]
	...
	equal representation to each class. \DIFaddbeginFL \DIFaddFL{The boxes represent the quartiles and
	the whiskers extend to 1.5 times of the interquartile range.}\DIFaddendFL }
...
\begin{figure}[tbp]
	...
	10 trials. \DIFaddbeginFL \DIFaddFL{The dashed blue line shows the performance obtained from
	using the whole training data.}\DIFaddendFL }
...
\begin{figure}[tbp]
	...
	curves are averaged over 10 trials. \DIFaddbeginFL \DIFaddFL{The dashed blue line shows the
	performance obtained from using the whole training data.}\DIFaddendFL }
...
\begin{figure}[tbp]
	\centering
	\includegraphics[width=0.7\textwidth]{figures/strengths-info_density-params}
	\caption[Effect of $\gamma$ in density weighting]{\DIFaddFL{Effect of $\gamma$ on
	}\textsc{\DIFaddFL{w-confidence}} \DIFaddFL{and }\textsc{\DIFaddFL{w-margin}} \DIFaddFL{using the glass and ionosphere
...
\begin{figure}[tbp]
	\centering
	\includegraphics[width=0.7\textwidth]{figures/strengths-thompson-params}
...
\begin{figure}[tbp]
	...
	\begin{subfigure}[t]{\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/selection-thompson-params-glass}
		\caption{\DIFaddFL{Glass dataset}}
		\label{fig:selection-thompson-params-glass}
	\end{subfigure}
	\begin{subfigure}[t]{\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/selection-thompson-params-ionosphere}
		\caption{\DIFaddFL{Ionosphere dataset}}
		\label{fig:selection-thompson-params-ionosphere}
    \end{subfigure}
	\caption[Effect of hyperparameters on Thompson sampling on heuristic
	selection]{\DIFaddFL{The effect of the the initial values of the parameters in
	...
	\DIFaddFL{(the purple line) and }\textsc{\DIFaddFL{qbb-kl}} \DIFaddFL{(the light green line) tend to get
	picked less often than others.}}
	\label{fig:selection-thompson-params}
\end{figure}

\begin{figure}[tbp]
	\centering
	\includegraphics[width=\textwidth]{figures/learning_curves-pool-mpba}
	\caption[Effect of the pool size]{\DIFaddFL{Effect of the
	pool size on the learning curves: We pick two large datasets---sdss and
	pageblocks---to investigate how the size of the pool affects the
	performance. The leftmost figures are the original learning curves
	from Figure \ref{fig:learning_curves-mpba-large} (we only show the
	first 200 examples so that all figures have the same scale). For the middle
	figures, we use the same test pool, but the unlabeled pool now only
	has a maximum of 300 candidates. Finally for the rightmost figures,
	the combined test pool and training pool have a size of 300.
	}}
