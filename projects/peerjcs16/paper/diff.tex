...
%DIF DEL combining-active-learners-v1.tex   Sun Feb 25 12:12:15 2018
%DIF ADD combining-active-learners-v2.tex   Fri May  4 11:17:31 2018
...
\hypersetup{
%DIF 28c28
%DIF < 	colorlinks   = true, % Colours links instead of ugly boxes
%DIF -------
	colorlinks   = true, % Colors links instead of ugly boxes %DIF >
%DIF -------
	citecolor    = {blue},
...
%DIF 122c122
%DIF < optimal heuristic for a particular source of data? Motivated by the success of
%DIF -------
best heuristic for a particular source of data? Motivated by the success of %DIF >
%DIF -------
...
\DIFdelbegin \DIFdel{In this paper we consider the binary and multiclass classification setting
...
over a finite label space $\Y$:
	...
	\DIFdel{h : \X \rightarrow p(\Y)
}\end{eqnarray*}
%DIFAUXCMD
\DIFdel{In other words, we require that the classifier produces class probability
...
with feature vector $\bm{x}$ belongs to the positive class with
	...
	\DIFdel{h(\bm{x}; \bm{\theta}) }&\DIFdel{= \Prob{y=1 \given \bm{x}; \bm{\theta}}
	= \dfrac{1}{1 + e^{-\bm{\theta}^T \bm{x}}}
}\end{eqnarray*}
%DIFAUXCMD
\DIFdel{and the training process involves learning the optimal weight vector
...
%DIFDELCMD < %%%
\DIFdelend The most common approach to producing a training set is passive learning, where
...
\DIFdelbegin \DIFdel{It seems
that we could }\DIFdelend \DIFaddbegin \DIFadd{Alternatively we can }\DIFaddend exploit the current set of labeled data to identify more
informative unlabeled examples to annotate. For instance we can pick examples
near the decision boundary of the classifier, where \DIFaddbegin \DIFadd{the class probability
estimates are uncertain (i.e. }\DIFaddend we are still unsure which class the example
belongs to\DIFdelbegin \DIFdel{(i.e. where the class probability estimates are
uncertain}\DIFdelend ).

\DIFdelbegin \DIFdel{This led to the development of many }\DIFdelend \DIFaddbegin \DIFadd{Many }\DIFaddend active learning heuristics \DIFdelbegin \DIFdel{that try }\DIFdelend \DIFaddbegin \DIFadd{have been developed }\DIFaddend to reduce the labeling
bottleneck without sacrificing the classifier performance. \DIFdelbegin \DIFdel{This is done by actively choosing }\DIFdelend \DIFaddbegin \DIFadd{These heuristics
actively choose }\DIFaddend the most informative examples to be labeled based on the
predicted class probabilities. Section~\ref{sec:heuristics} describes two
families of algorithms in \DIFdelbegin \DIFdel{detail---uncertainty }\DIFdelend \DIFaddbegin \DIFadd{detail: uncertainty }\DIFaddend sampling and version space
reduction.
...
learning suggestions with bandit and \DIFdelbegin \DIFdel{social choice theories }\DIFdelend \DIFaddbegin \DIFadd{rank aggregation methods }\DIFaddend in
Section~\ref{sec:expert}.
\DIFaddbegin

\DIFaddend The use of bandit algorithms \DIFdelbegin \DIFdel{, where we only pick one
heuristic at each time step, }\DIFdelend \DIFaddbegin \DIFadd{to combine active learners }\DIFaddend has been studied
before~\citep{baram04, hsu15}. \DIFdelbegin \DIFdel{However, as }\DIFdelend \DIFaddbegin \DIFadd{Borda count, a simple rank aggregation method,
has been used in the context of multi-task learning for linguistic annotations
\mbox{%DIFAUXCMD
\citep{reichart08}}\hspace{0pt}%DIFAUXCMD
, where we have one active learner selecting examples to
...
\citep{reyes18} }\hspace{0pt}%DIFAUXCMD
to combine uncertainty information from multiple
labels. As }\DIFaddend far as we know, \DIFdelbegin \DIFdel{this }\DIFdelend \DIFaddbegin \DIFadd{other aggregation methods have not been explored
and our work }\DIFaddend is the first time that social choice theory is used to rank and
aggregate suggestions \DIFaddbegin \DIFadd{from multiple active learners}\DIFaddend .

\DIFdelbegin \DIFdel{To compare the algorithms, we run them on }\DIFdelend \DIFaddbegin \DIFadd{This paper makes the following two main contributions:
}\begin{enumerate}
	\item \DIFadd{We empirically compare 4 bandit and 3 rank aggregation algorithms in
	the context of combining active learning heuristics. We apply these
	algorithms to }\DIFaddend 11 benchmark datasets from the UCI Machine Learning
	...
	described \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{from }\DIFaddend Section~\ref{sec:expt} \DIFdelbegin \DIFdel{and~\ref{sec:discuss}.
	}\DIFdelend \DIFaddbegin \DIFadd{to~\ref{sec:discussion}.
	}\item \DIFadd{We propose two metrics for evaluation: the mean posterior balanced
	accuracy (MPBA) and the strength of an algorithm. The MPBA extends the
	metric proposed in \mbox{%DIFAUXCMD
\cite{brodersen10} }\hspace{0pt}%DIFAUXCMD
from the binary to the multi-class
	...
	in \mbox{%DIFAUXCMD
\cite{baram04} }\hspace{0pt}%DIFAUXCMD
which evaluates the performance of an active learner or
	combiner, relative to passive learning. The main difference between our
	measure and that of \mbox{%DIFAUXCMD
\cite{baram04} }\hspace{0pt}%DIFAUXCMD
is that ours assigns a higher number for
...
\DIFdelbegin \DIFdel{Recall that in }\DIFdelend \DIFaddbegin \DIFadd{In this paper we consider the binary and multiclass classification settings
where we would like to learn a classifier $h$, which is a function that maps
some feature space $\X \subseteq \mathbb{R}^d$ to a probability distribution
over a finite label space $\Y$:
}\begin{align}
	\DIFadd{h : \X \rightarrow p(\Y)
}\end{align}
\DIFadd{In other words, we require that the classifier produces class probability
...
object with feature vector $\bm{x}$ belongs to the positive class with
	...
	\DIFadd{h(\bm{x}; \bm{\theta}) }&\DIFadd{= \Prob{y=1 \given \bm{x}; \bm{\theta}}
	= \dfrac{1}{1 + e^{-\bm{\theta}^T \bm{x}}}
}\end{align}
\DIFadd{and the optimal weight vector $\bm{\theta}$ is learned in training. We can
...
\DIFadd{In }\DIFaddend active learning, we \DIFdelbegin \DIFdel{exploit }\DIFdelend \DIFaddbegin \DIFadd{use }\DIFaddend the class probability estimates from a trained
classifier to estimate a score of informativeness for each \DIFdelbegin \DIFdel{test
}\DIFdelend \DIFaddbegin \DIFadd{unlabeled }\DIFaddend example.
...
been labeled. In practice \DIFdelbegin \DIFdel{, }\DIFdelend this normally means that we label a small random
sample at the beginning. These become the labeled training set $\Labeled_T
\subset \X \times \Y$ \DIFdelbegin \DIFdel{, }\DIFdelend and the rest form the unlabeled set $\Unlabeled \subset
\X$.
...
%DIFDELCMD <     	unlabeled set $\Unlabeled$,
%DIFDELCMD <         labeled training set $\Labeled_T$,
%DIFDELCMD <         classifier $h(\bm{x})$,
%DIFDELCMD <         and active learner $r(\bm{x}; h)$.
%DIFDELCMD <         }
%DIFDELCMD <     %%%
\DIFdelend \DIFaddbegin \KwIn{
    ...
    \DIFaddend \BlankLine
	\DIFdelbegin %DIFDELCMD < \Repeat{we have enough training examples.}{
%DIFDELCMD < 		Select the most informative candidate $\bm{x}_*$ from $\Unlabeled$ using the
%DIFDELCMD < 			active learning rule $r(\bm{x}; h)$. \;
%DIFDELCMD <         Ask the expert to label $\bm{x}_*$. Call the label $y_*$. \;
%DIFDELCMD < 		Add the newly labeled example to the training set:
%DIFDELCMD < 			$\Labeled_T \leftarrow \Labeled_T  \cup \{(\bm{x}_*, y_*)\}$. \;
%DIFDELCMD < 		Remove the newly labeled example from the unlabeled set:
%DIFDELCMD < 			$\Unlabeled \leftarrow \Unlabeled \setminus \{ \bm{x}_* \}$. \;
%DIFDELCMD <         Retrain the classifier $h(\bm{x})$ using $\Labeled_T$. \;
%DIFDELCMD <     }
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \Repeat{we have enough training examples.}{
...
\DIFaddend There are also heuristics that involve minimizing the variance or maximizing
the classifier certainty of the model \citep{schein07}, but \DIFdelbegin \DIFdel{we do not }\DIFdelend \DIFaddbegin \DIFadd{they are
...
\mbox{%DIFAUXCMD
\citep{tran15}}\hspace{0pt}%DIFAUXCMD
, so we do not }\DIFaddend consider them in this paper\DIFdelbegin \DIFdel{as they are
computationally inefficient}\DIFdelend .

\DIFaddbegin \DIFadd{A more comprehensive treatment of these active learning heuristics can be found
in \mbox{%DIFAUXCMD
\cite{settles12}}\hspace{0pt}%DIFAUXCMD
.
}

\DIFaddend \subsection{Uncertainty Sampling}~\label{subsec:uncertainty}
\cite{lewis94} introduced \DIFdelbegin \DIFdel{the idea of }\DIFdelend uncertainty sampling, where we select the instance
...
simplest way to quantify uncertainty is \DIFdelbegin \DIFdel{~\mbox{%DIFAUXCMD
\cite{culotta05}}\hspace{0pt}%DIFAUXCMD
's }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend least confidence
heuristic\DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\citep{culotta05}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend , where we pick the candidate whose most likely
label the classifier is most uncertain about:
\begin{align}
	r_{LC}(\DIFdelbegin %DIFDELCMD < \bm{x}%%%
...
\begin{align}
	r_{HE}(\DIFdelbegin %DIFDELCMD < \bm{x}%%%
\DIFdelend \DIFaddbegin \DIFadd{\Unlabeled}\DIFaddend ; h) &= \argmax_{\bm{x} \in \Unlabeled}
...
\begin{align}
	r_{SM}(\DIFdelbegin %DIFDELCMD < \bm{x}%%%
\DIFdelend \DIFaddbegin \DIFadd{\Unlabeled}\DIFaddend ; h) &= \argmax_{\bm{x} \in \Unlabeled}
...
the margin is, the \DIFdelbegin \DIFdel{more uncertain we are about the object's class
membership }\DIFdelend \DIFaddbegin \DIFadd{harder it is to differentiate between the two most likely
labels.
}

\DIFadd{An extension to the above three heuristics is to weight the score with the
information density so that we give more importance to instances in regions of
high density:
}\begin{align}
	\DIFadd{s_{ID}(\Unlabeled; h) }&\DIFadd{=
		\bigg(\dfrac{1}{U} \sum_{k = 1}^E
		\text{sim}(\bm{x}, \bm{x}^{(k)})\bigg) s(\bm{x}; h)
}\end{align}
\DIFadd{where $h$ is the classifier, $s(\bm{x}; h)$ is the original score function of
...
$\gamma$:
	...
	\DIFadd{\text{sim}(\bm{x}, \bm{x}^{(k)})
		}&\DIFadd{= \exp(\gamma ||\bm{x} - \bm{x}^{(k)}||^2) \label{eqn:kernel}
}\end{align}
\DIFadd{The information density weighting was proposed by~\mbox{%DIFAUXCMD
\cite{settles08} }\hspace{0pt}%DIFAUXCMD
to
...
not affect the classifier performance on the data as a whole}\DIFaddend .
...
\begin{align}
	r_{QBBM}(\DIFdelbegin %DIFDELCMD < \bm{x}%%%
\DIFdelend \DIFaddbegin \DIFadd{\Unlabeled}\DIFaddend ; h) &= \argmax_{\bm{x} \in \Unlabeled}
...
\begin{align}
	r_{QBBKL}(\DIFdelbegin %DIFDELCMD < \bm{x}%%%
\DIFdelend \DIFaddbegin \DIFadd{\Unlabeled}\DIFaddend ; h) &= \argmax_{\bm{x} \in \Unlabeled}
	\left\{ \dfrac{1}{B}
...
Out of the five heuristics discussed, \DIFdelbegin \DIFdel{how do we know which one is the most
optimal one}\DIFdelend \DIFaddbegin \DIFadd{which one should we use in practice when
we would like to apply active learning to a particular problem}\DIFaddend ? There have been
some attempts in the literature to do a theoretical analysis \DIFaddbegin \DIFadd{of their
performance}\DIFaddend . Proofs are however scarce, and when there is one available, they
...
heuristic. In this paper we explore two different approaches\DIFdelbegin \DIFdel{. We }\DIFdelend \DIFaddbegin \DIFadd{: we }\DIFaddend can either
...
probability theory \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{berry85}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . The colorful name originates from the
...
maximize the gambler's lifetime rewards\DIFdelbegin \DIFdel{while minimizing the number of pulls}\DIFdelend . In the context of active learning,
...
By always exploiting, we could miss out on the \DIFdelbegin \DIFdel{optimal }\DIFdelend \DIFaddbegin \DIFadd{best }\DIFaddend heuristic. On the other
...
but only the reward received from using any of them. \DIFdelbegin \DIFdel{Based }\DIFdelend \DIFaddbegin \DIFadd{At each time step, we
receive a reward from a heuristic, and based }\DIFaddend on the history of all the rewards,
the bandit algorithm can decide on which heuristic to pick next. Formally, we
need to learn the function
\begin{align}
	b : \DIFaddbegin \DIFadd{(}{\DIFadd{J_\R}} \DIFadd{\times }\DIFaddend [0, 1]\DIFdelbegin \DIFdel{^{R \times T} }\DIFdelend \DIFaddbegin \DIFadd{)^{n} }\DIFaddend \rightarrow J_\R
\end{align}
where $b$ is the bandit algorithm, $[0, 1]$ is a normalized reward between
0 and 1, $J_\R$ is the index set over the set of heuristics $\R$, and \DIFdelbegin \DIFdel{$T$
}\DIFdelend \DIFaddbegin \DIFadd{$n$
}\DIFaddend is the time horizon.
...
bootstrap on $\Labeled_T$ to estimate the \DIFdelbegin \DIFdel{generalisation }\DIFdelend \DIFaddbegin \DIFadd{generalization }\DIFaddend performance. However
...
automate the selection process. \DIFdelbegin \DIFdel{~}\DIFdelend \DIFaddbegin \DIFadd{They proposed a reward called the
...
(SVMs). We will not compare our results directly with those in \mbox{%DIFAUXCMD
\cite{baram04}
...
produce unnormalized scores. }\DIFaddend \cite{hsu15} studied an improved version \DIFdelbegin \DIFdel{,
}\DIFdelend \DIFaddbegin \DIFadd{of EXP4,
called }\DIFaddend EXP4.P, \DIFdelbegin \DIFdel{along with }\DIFdelend \DIFaddbegin \DIFadd{and used }\DIFaddend importance weighting to estimate the \DIFdelbegin \DIFdel{rewards }\DIFdelend \DIFaddbegin \DIFadd{true classifier
performance }\DIFaddend using only the training set. \DIFdelbegin \DIFdel{This paperempirically compares }\DIFdelend \DIFaddbegin \DIFadd{In this paper, we empirically compare
}\DIFaddend the following four \DIFaddbegin \DIFadd{bandit }\DIFaddend algorithms: Thompson sampling, OC-UCB, kl-UCB, and
EXP3++.

%DIFDELCMD < %%%
\DIFdelend \begin{figure}[p]
	\centering
	\DIFdelbeginFL %DIFDELCMD < \begin{tikzpicture}[auto, node distance=2cm,>=latex', scale=0.8, transform shape]
...
%DIFDELCMD < 		\draw [active] (pool) -- node[name=training] {$\Labeled_T$} (classifier);
%DIFDELCMD < 		\draw [active] (classifier) -- node[name=y] {$p(\Y)$} (heuristic);
%DIFDELCMD < 		\draw [active] (heuristic) -- node[name=y] {} (best);
%DIFDELCMD < 		\draw [active] (best) |- node[name=x] {$\bm{x}_*$} (oracle);
%DIFDELCMD < 		\draw [active] (oracle) -- node[name=xy] {$(\bm{x}_*, y_*)$} (pool);
%DIFDELCMD <

%DIFDELCMD < 		\draw [bandit] (test input) -- node[name=test] {$\Labeled_S$} (classifier);
%DIFDELCMD < 		\draw [active] (pool) -- node[right, name=training] {$\Unlabeled$} (classifier);
%DIFDELCMD < 		\draw [bandit] (classifier) -- node[name=reward] {reward $w$} (bandit);
%DIFDELCMD < 		\draw [bandit] (heuristic input) -- node[name=heuristics] {$\R$} (bandit);
%DIFDELCMD < 		\draw [bandit] (bandit) -- node[name=heuristics] {} (best heuristic);
%DIFDELCMD < 		\draw [bandit, dashed] (best heuristic) -- (heuristic);
%DIFDELCMD <

%DIFDELCMD < 	\end{tikzpicture}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics{Fig1}
	\DIFaddendFL \caption{\DIFdelbeginFL \DIFdelFL{In the active }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Active }\DIFaddendFL learning pipeline with bandit algorithms\DIFdelbeginFL \DIFdelFL{, we
	}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{. We
	}\DIFaddendFL need to collect rewards $w$ from the test set $\Labeled_S$ in order to
...
\begin{algorithm}[p]
    ...
    \BlankLine
	\DIFdelbegin %DIFDELCMD < \While{$|\Labeled_T| < n$}{
%DIFDELCMD < 		Select a heuristic $r_* \in \R$ according to \Select. \;
%DIFDELCMD < 		Select the most informative candidate $\bm{x}_*$ from $\Unlabeled$ using the
%DIFDELCMD < 			chosen heuristic $r_*(\bm{x}; h)$. \;
%DIFDELCMD <         Ask the expert to label $\bm{x}_*$. Call the label $y_*$. \;
%DIFDELCMD < 		Add the newly labeled example to the training set:
%DIFDELCMD < 			$\Labeled_T \leftarrow \Labeled_T  \cup \{(\bm{x}_*, y_*)\}$. \;
%DIFDELCMD < 		Remove the newly labeled example from the unlabeled set:
%DIFDELCMD < 			$\Unlabeled \leftarrow \Unlabeled \setminus \{ \bm{x}_* \}$. \;
%DIFDELCMD < 		Retrain the classifier $h(\bm{x})$ using $\Labeled_T$.\;
%DIFDELCMD < 		Run the updated classifier on the test set $\Labeled_S$ to compute the
%DIFDELCMD < 			increase in the performance $w$. \;
%DIFDELCMD <         Update the parameters of $B$ with \Update$(w)$. \;
%DIFDELCMD <     }
%DIFDELCMD < 	%%%
\DIFdelend \DIFaddbegin \While{$|\Labeled_T| < n$}{
	...
	\DIFaddend \caption{Pool-based active learning with bandit theory. Note that in
...
even with the \DIFdelbegin \DIFdel{optimal heuristic, the
classifier trained on finite data might not be the right one, so we still cannot }\DIFdelend \DIFaddbegin \DIFadd{best heuristic, we still might not }\DIFaddend score perfectly due to having
a poor classifier \DIFaddbegin \DIFadd{trained on finite data}\DIFaddend . Conversely, a bad heuristic might be
...
$W_i$ as a \DIFdelbegin \DIFdel{random variable and assume the errors to be
normally distributed }\DIFdelend \DIFaddbegin \DIFadd{normally distributed random variable }\DIFaddend with mean $\nu_i$ and variance
$\tau_i^2$:
	\begin{align}
		(W_i \mid \nu_i) \sim \Normal(\nu_i, \tau_i^2)
	\end{align}
...
We start with a prior \DIFdelbegin \DIFdel{knowledge of }\DIFdelend \DIFaddbegin \DIFadd{on }\DIFaddend $\mu_i$ and $\sigma_i^2$ for each heuristic $r_i$. The
choice of prior \DIFdelbegin \DIFdel{do }\DIFdelend \DIFaddbegin \DIFadd{does }\DIFaddend not usually matter in the long run. Since initially we do
...
that maximizes the following upper bound:
    \begin{align}
		r_* = \argmax_{i} \DIFaddbegin \Bigg( \DIFaddend \overline{w_i} +
			  \sqrt{\dfrac{\alpha}{T_i(t)} \ln\Big(\dfrac{\psi n}{t}\Big)} \DIFaddbegin \Bigg)
    \DIFaddend \end{align}
where $\overline{w_i}$ is the average of the rewards from $r_i$ that we \DIFdelbegin \DIFdel{observe
}\DIFdelend \DIFaddbegin \DIFadd{have
observed }\DIFaddend so far, $t$ is the time step, $T_i(t)$ is the number times we have
...
\DIFaddbegin \DIFadd{In kl-UCB, }\DIFaddend \cite{cappe13} suggest that we \DIFaddbegin \DIFadd{can }\DIFaddend instead consider the
KL-divergence \DIFdelbegin \DIFdel{when finding
the }\DIFdelend \DIFaddbegin \DIFadd{between the distribution of the current estimated reward and that
of the }\DIFaddend upper bound. In the case of normally distributed rewards with known
variance $\sigma^2$, the chosen heuristic would be
    \begin{align}
		r_* = \argmax_{i} \DIFaddbegin \Bigg( \DIFaddend \overline{w_i} +
			  \sqrt{ 2 \sigma^2 \dfrac{\ln\big(T_i(t)\big)}{t} } \DIFaddbegin \Bigg)
    \DIFaddend \end{align}
\DIFdelbegin \DIFdel{Algorithm}\DIFdelend \DIFaddbegin \DIFadd{Algorithms}\DIFaddend ~\ref{alg:oc-ucb} and~\ref{alg:kl-ucb} summarize these two UCB
...
\begin{algorithm}[htbp]
	...
	\caption{Optimally Confident UCB. Notations:
			 \DIFdelbegin \DIFdel{$\R$ is the set of heuristics,
			 }\DIFdelend $n$ is the time horizon (maximum number of \DIFaddbegin \DIFadd{time }\DIFaddend steps), $t$ is the
			 current time step, $T_i(t)$ counts of how many times heuristic $i$
			 has been selected \DIFaddbegin \DIFadd{before step $t$}\DIFaddend , $w$ is the reward received, and
...
\begin{algorithm}[htbp]
	...
	\DIFdelbegin \DIFdel{$\R$ is the
	set of heuristics, }\DIFdelend $\sigma$ is the variance of the rewards, $t$ is the
	current time step, $T_i(t)$ counts of how many times heuristic $i$ has been
	selected \DIFaddbegin \DIFadd{before step $t$}\DIFaddend , $w$ is the reward received, and $\overline{w_i}$ is the
...
The \DIFdelbegin \DIFdel{EXP3 algorithm }\DIFdelend \DIFaddbegin \DIFadd{exponential-weight algorithm for exploration and exploitation (EXP3) }\DIFaddend was
...
construct the distribution with exponential weighting (hence the name EXP3\DIFdelbegin \DIFdel{, which stands for exponential-weight algorithm for exploration and
exploitation}\DIFdelend ). We
...
\begin{algorithm}[htbp]
	...
	to compute the loss $L_i$, \DIFaddbegin \DIFadd{$\rho$ is the distribution from which a heuristic
	is sampled, }\DIFaddend and $w$ is the reward received.}
	\label{alg:exp3pp}
\end{algorithm}

\subsection{Combining Suggestions with Social Choice Theory}

A drawback of the \DIFaddbegin \DIFadd{above }\DIFaddend bandit methods is that at each iteration, we could only
use one suggestion from one particular heuristic. \DIFdelbegin \DIFdel{In addition, we also }\DIFdelend \DIFaddbegin \DIFadd{EXP4 and EXP4.P algorithms
...
reward is the performance on a test set, we would }\DIFaddend need to keep around a
\DIFdelbegin \DIFdel{test set}\DIFdelend \DIFaddbegin \DIFadd{separate subset of the data}\DIFaddend , which is expensive and sometimes impossible to
obtain in practice.  \DIFdelbegin \DIFdel{Since most heuristics require little computation, we could in fact
run all of them at the same time and then combine the suggestions. }\DIFdelend This leads us to social choice theory\DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{, which can combine
suggestions like EXP4 and EXP4.P, while not needing the concept of a reward.
}\DIFaddend Originally developed by political scientists like Nicolas de Condorcet and
...
ranking of the candidates, which is achieved by a function \DIFdelbegin \DIFdel{$k(s(\bm{x}; h))$
that converts the scores into a preference ordering:
	}\begin{eqnarray*}
		\DIFdel{k : \mathbb{R}^{U} \rightarrow \sigma(J_\Unlabeled)
	}\end{eqnarray*}
%DIFAUXCMD
\DIFdel{where $\sigma(J_\Unlabeled)$ is a permutation over the index set }\DIFdelend \DIFaddbegin \DIFadd{$k(s, \Unlabeled)$
that provides a ranking }\DIFaddend of the unlabeled \DIFdelbegin \DIFdel{pool.
$\Unlabeled$. }\DIFdelend \DIFaddbegin \DIFadd{examples according to their scores.
}\DIFaddend For example, $k$ could assign the candidate with the highest score a rank of 1,
...
will then combine all the rankings into a combined ranking,
	...
	\end{align}
\DIFdelbegin \DIFdel{from which }\DIFdelend \DIFaddbegin \DIFadd{where $\sigma(J_\Unlabeled)$ is a permutation over the index set of the
unlabeled pool $\Unlabeled$ and $R$ is the number of heuristics. From these }\DIFaddend we
can pick the highest-ranked candidate to annotate. See Table~\ref{tab:rank}
for an \DIFdelbegin \DIFdel{exmaple}\DIFdelend \DIFaddbegin \DIFadd{example}\DIFaddend .
...
\begin{table}[htbp]
	...
	\begin{tabular}{llllll}
		...
		\midrule
		Rank & \DIFdelbeginFL \DIFdelFL{$k(s(\bm{x}; h))$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$k(s, \Unlabeled)$ }\DIFaddendFL & 4 & 1 & 3 & 2 \\
		\bottomrule
	\end{tabular}
\end{table}
...
%DIFDELCMD < 		\draw [active] (pool) -- node[right, name=training] {$\Unlabeled$} (classifier);
%DIFDELCMD < 		\draw [active] (classifier) -- node[name=y] {$p(\Y)$} (heuristic);
%DIFDELCMD < 		\draw [active] (heuristic) -- node[name=y] {} (rank);
%DIFDELCMD < 		\draw [active] (rank) -- node[name=x] {$\sigma_1(J_\Unlabeled), ..., \sigma_R(J_\Unlabeled)$} (aggregate);
%DIFDELCMD < 		\draw [active] (aggregate) -- node[name=x] {$\sigma(J_\Unlabeled)$} (aggregated);
%DIFDELCMD < 		\draw [active] (aggregated) -- node[name=x] {$\bm{x}_*$} (oracle);
%DIFDELCMD < 		\draw [active] (oracle) -- node[name=xy] {$(\bm{x}_*, y_*)$} (pool);
%DIFDELCMD < 		\draw [bandit] (heuristic input) -- node[name=heuristics] {$\R$} (heuristic);
%DIFDELCMD < 	\end{tikzpicture}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics{Fig2}
	\DIFaddendFL \caption{\DIFdelbeginFL \DIFdelFL{In the active }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Active }\DIFaddendFL learning pipeline with rank aggregation methods\DIFaddbeginFL \DIFaddFL{. Unlike the
	bandit pipeline}\DIFaddendFL , there is only one cycle \DIFdelbeginFL \DIFdelFL{, }\DIFdelendFL in which we aggregate information
	from all \DIFdelbeginFL \DIFdelFL{heursitics}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{heuristics}\DIFaddendFL . Additional notation: $\sigma(J_\Unlabeled)$ is a
...
%DIFDELCMD <     	unlabeled set $\Unlabeled$,
%DIFDELCMD <         labeled training set $\Labeled_T$,
%DIFDELCMD <         classifier $h$,
%DIFDELCMD <         set of active learning suggestions $\R$,
%DIFDELCMD <         and rank aggregator $(c \circ k)$.
%DIFDELCMD <         }
%DIFDELCMD <     %%%
\DIFdelend \DIFaddbegin \KwIn{
    	unlabeled set $\Unlabeled$,
    ...
    \DIFaddend \BlankLine
	\DIFdelbegin %DIFDELCMD < \Repeat{we have enough training examples.}{
%DIFDELCMD <         \For{$r \in \R$}{
%DIFDELCMD <         	Rank all the candidates in $\Unlabeled$. \;
%DIFDELCMD <         }
%DIFDELCMD <         Aggregate all the rankings into one ranking using the aggregator $c$. \;
%DIFDELCMD <         Select the highest-ranked candidate $\bm{x}_*$ from $\Unlabeled$. \;
%DIFDELCMD < 		Ask the expert to label $\bm{x}_*$. Call the label $y_*$. \;
%DIFDELCMD < 		Add the newly labeled example to the training set:
%DIFDELCMD < 			$\Labeled_T \leftarrow \Labeled_T  \cup \{(\bm{x}_*, y_*)\}$. \;
%DIFDELCMD < 		Remove the newly labeled example from the unlabeled set:
...
\DIFdelend \DIFaddbegin \Repeat{we have enough training examples.}{
    ...
    \DIFaddend \caption{Pool-based active learning with social choice theory\DIFaddbegin \DIFadd{.}\DIFaddend }
    \label{alg:choice}
\end{algorithm}
...
maximizes the Spearman correlation \DIFdelbegin \DIFdel{bewteen }\DIFdelend \DIFaddbegin \DIFadd{between }\DIFaddend the ranks. Note that this method
...
The third approach we consider is the Schulze method~\citep{schulze11}. \DIFdelbegin \DIFdel{This
}\DIFdelend \DIFaddbegin \DIFadd{Out of
...
}\DIFaddend method is more computationally intensive since it requires examining all pairs
...
algorithm involves finding the path of the \DIFdelbegin \DIFdel{strongest }\DIFdelend \DIFaddbegin \DIFadd{maximal }\DIFaddend strength from each
...
x_j) \geq p(x_j, x_i)$ for all other $x_j$. This problem has a similar \DIFdelbegin \DIFdel{flavour
}\DIFdelend \DIFaddbegin \DIFadd{flavor
}\DIFaddend to the problem of finding the shortest path. In fact, the implementation uses a
...
We use 11 \DIFaddbegin \DIFadd{classification }\DIFaddend datasets taken from the UCI Machine Learning
Repository\DIFdelbegin \DIFdel{~\mbox{%DIFAUXCMD
\citep{lichman13}}\hspace{0pt}%DIFAUXCMD
,
along }\DIFdelend \DIFaddbegin \footnote{\DIFadd{https://archive.ics.uci.edu/ml/}} \DIFadd{\mbox{%DIFAUXCMD
\citep{lichman13}}\hspace{0pt}%DIFAUXCMD
,
}\DIFaddend with a large \DIFaddbegin \DIFadd{multiclass classification }\DIFaddend dataset which we extracted from the SDSS
project\DIFaddbegin \footnote{\DIFadd{http://dx.doi.org/10.5281/zenodo.58500}} \DIFaddend \citep{alam15}. The
code for the experiments can be found on our GitHub
repository\footnote{\DIFdelbegin %DIFDELCMD < \url{https://github.com/chengsoonong/mclass-sky}%%%
\DIFdelend \DIFaddbegin \DIFadd{https://github.com/chengsoonong/mclass-sky}\DIFaddend }.
...
class and the maximum achievable performance using logistic regression. \DIFaddbegin \DIFadd{These
...
\DIFaddbeginFL
	...
	\DIFaddendFL \caption {Overview of datasets\DIFaddbeginFL \DIFaddFL{. The following datasets are from the UCI
	...
	dataset was extracted from Data Release 12 of SDSS-III.}\DIFaddendFL }
	\label{tab:datasets}
\DIFaddbeginFL
...
For each dataset, we \DIFdelbegin \DIFdel{build a classifier to predict the class of an instance
based on its features. We use a }\DIFdelend \DIFaddbegin \DIFadd{use Scikit-learn \mbox{%DIFAUXCMD
\citep{pedregosa11} }\hspace{0pt}%DIFAUXCMD
to train a logistic
regression model using a }\DIFaddend 10-fold stratified shuffled \DIFdelbegin \DIFdel{split cross
validation.
The stratifiedstrategy }\DIFdelend \DIFaddbegin \DIFadd{cross-validation.
Here `stratified' }\DIFaddend means that the proportion of the classes remains constant in each
split. We standardize all features to have zero mean and unit variance.
Although all examples have already been labeled, we simulate \DIFdelbegin \DIFdel{active learning }\DIFdelend \DIFaddbegin \DIFadd{the active
learning task }\DIFaddend by assuming that certain examples do not have any labels. \DIFdelbegin \DIFdel{In
particular}\DIFdelend \DIFaddbegin \DIFadd{For
each fold}\DIFaddend , the unlabeled pool size is 70\% of data up to a maximum of 10,000
examples\DIFdelbegin \DIFdel{. The }\DIFdelend \DIFaddbegin \DIFadd{, and the }\DIFaddend test pool consists of the remaining examples up to a maximum
of 20,\DIFdelbegin \DIFdel{000---we assume labels are available here}\DIFdelend \DIFaddbegin \DIFadd{000. We assume all test examples are labeled}\DIFaddend . We initialize the
classifier by labeling 10 random instances and using them as the initial
training set. \DIFdelbegin \DIFdel{Note
that in practice, there can be a large amount of unlabeled data, so for
computational reasons, we only }\DIFdelend \DIFaddbegin \DIFadd{The heuristics are fast enough such that we can }\DIFaddend assign a score to
\DIFdelbegin \DIFdel{a small random sample from the
unlabeled pool. In particular, we feed 300 randomly selected candidates from
the unlabeled pool into the algorithm. }\DIFdelend \DIFaddbegin \DIFadd{every unlabeled instance at every time step. }\DIFaddend We use logistic regression with a
Gaussian kernel approximation and an L2 \DIFdelbegin \DIFdel{loss}\DIFdelend \DIFaddbegin \DIFadd{regularizer}\DIFaddend . In the binary case, the
loss function is
\begin{align}
	L &= \dfrac{1}{2} \DIFdelbegin %DIFDELCMD < \bm{w}%%%
\DIFdelend \DIFaddbegin \bm{\theta}\DIFaddend ^T \bm{\theta} + C \sum_{i = 1}^n
	      \ln\Big(1 + \exp(-y_i(\bm{\theta}^T f(\bm{x}_i)))\Big)
\end{align}
where $\bm{x}_i$ is the feature vector of the $i$th example, $y_i \in \{0, 1\}$
is the label of $\bm{x}_i$, and $n$ is the training size. The \DIFdelbegin \DIFdel{first term
of the
sum }\DIFdelend \DIFaddbegin \DIFadd{term
$\dfrac{1}{2} \bm{\theta}^T \bm{\theta}$ }\DIFaddend is the regularization term to ensure
that the weight vector $\bm{\theta}$ is not too large, and $C$ is \DIFdelbegin \DIFdel{another regularization hyperparameter which we
simply set to 1000. }\DIFdelend \DIFaddbegin \DIFadd{a
regularization hyperparameter in $[10^{-6}, 10^6]$ which we find using grid
search. }\DIFaddend To speed up the training time while using the Gaussian kernel, we
...
the One-vs-Rest strategy, where for every class \DIFdelbegin \DIFdel{, }\DIFdelend we build a binary classifier
...
given a sample of \DIFaddbegin \DIFadd{maximum }\DIFaddend 100 examples that have already been labeled.

For the bandit algorithms, \DIFaddbegin \DIFadd{we use }\DIFaddend the increase in the mean posterior balanced
accuracy (MPBA) on the test set \DIFdelbegin \DIFdel{is used }\DIFdelend as the reward. The MPBA can be thought of as
...
this metric takes into account \DIFdelbegin \DIFdel{of the }\DIFdelend class imbalance. This is because we first
calculate the recall \DIFdelbegin \DIFdel{rate }\DIFdelend in each class and then take the average, thus giving each
...
In total, we test 17 \DIFdelbegin \DIFdel{methods}\DIFdelend \DIFaddbegin \DIFadd{query strategies}\DIFaddend . This includes passive learning, 8 active
learning heuristics, 5 bandit algorithms, and 3 aggregation methods. \DIFaddbegin \DIFadd{The bandit
algorithms include the four described in Section~\ref{subsec:bandit} and a
baseline called }\textsc{\DIFadd{explore}} \DIFadd{which simply selects a random heuristic at
...
representative experts: }\textsc{\DIFadd{passive}}\DIFadd{, }\textsc{\DIFadd{confidence}}\DIFadd{, }\textsc{\DIFadd{margin}}\DIFadd{,
}\textsc{\DIFadd{entropy}}\DIFadd{, }\textsc{\DIFadd{qbb-margin}}\DIFadd{, and }\textsc{\DIFadd{qbb-kl}}\DIFadd{. We have not
explored how adding the heuristics with information density weighting to the
bandits would impact the performance. }\DIFaddend See Table~\ref{tab:abbre} for a list of
...
heuristics, we also test a slight variant (}\textsc{\DIFdel{w-confidence}}%DIFAUXCMD
\DIFdel{,
}\textsc{\DIFdel{w-margin}}%DIFAUXCMD
\DIFdel{, }\textsc{\DIFdel{w-entropy}}%DIFAUXCMD
...
density weighting is proposed by~\mbox{%DIFAUXCMD
\cite{settles08} }\hspace{0pt}%DIFAUXCMD
...
}\textsc{\DIFdel{passive}}%DIFAUXCMD
\DIFdel{, }\textsc{\DIFdel{confidence}}%DIFAUXCMD
\DIFdel{, }\textsc{\DIFdel{margin}}%DIFAUXCMD
\DIFdel{, }\textsc{\DIFdel{entropy}}%DIFAUXCMD
\DIFdel{,
}\textsc{\DIFdel{qbb-margin}}%DIFAUXCMD
\DIFdel{, and }\textsc{\DIFdel{qbb-kl}}%DIFAUXCMD
...
%DIFDELCMD < %%%
\DIFdelend Given that there are 12 datasets, each with 17 learning curves, we need a
...
strength of an active learner or a combiner \DIFdelbegin \DIFdel{, }\DIFdelend relative to passive learning \DIFdelbegin \DIFdel{, }\DIFdelend as
\begin{align}
    \text{Strength}(h; m) &=
...
possible performance\DIFdelbegin \DIFdel{achieved from using all labeled data in the training set}\DIFdelend \DIFaddbegin \footnote{\DIFadd{The best possible performance in each trial is
...
of all the methods.}}\DIFaddend , and $m(h, t)$ is the performance achieved using the first
...
\DIFaddbegin \begin{figure}[htbp]
	...
	\caption[Strength Measure]{\DIFaddFL{An illustration of the MPBA strength measure. It
...
\DIFaddend We evaluate the algorithm performance with two metrics: the accuracy score and
the MPBA. The accuracy score is \DIFdelbegin \DIFdel{simply the proportion }\DIFdelend \DIFaddbegin \DIFadd{the percentage }\DIFaddend of instances in the test set
...
weight \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{on }\DIFaddend each class and thus favors algorithms that can predict the label of
all classes equally well.

\DIFdelbegin %DIFDELCMD < \begin{figure}[htbp]
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{The heuristics with information density weighting and Thompson sampling have a
...
iterate through many hyperparameter values quickly. With }\textsc{\DIFaddFL{w-confidence}}\DIFaddFL{,
}\textsc{\DIFaddFL{w-margin}}\DIFaddFL{, and }\textsc{\DIFaddFL{w-entropy}}\DIFaddFL{, we set $\gamma$ in the Gaussian
...
\ref{fig:gamma}. For }\textsc{\DIFaddFL{thompson}}\DIFaddFL{, the prior values for $\mu$, $\sigma^2$
...
\begin{figure}[p]
	\DIFaddendFL \centering
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.7\textwidth]{figures/strength}
%DIFDELCMD < 	\caption[Strength Measure]{%%%
\DIFdelFL{An illustration }\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.7\textwidth]{Fig4}
	\caption[Effect of $\gamma$ in density weighting]{\DIFaddFL{Effect }\DIFaddendFL of \DIFdelbeginFL \DIFdelFL{the strength measure. It is
	proportional to the shaded area between the (red) passive learning curve
	}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\gamma$ on
	}\textsc{\DIFaddFL{w-confidence}} \DIFaddendFL and \DIFaddbeginFL \textsc{\DIFaddFL{w-margin}} \DIFaddFL{using }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{(blue) active learning curve}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{glass and ionosphere
	datasets}\DIFaddendFL . \DIFdelbeginFL \DIFdelFL{The bigger }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{We examine six different values for $\gamma$: }\DIFaddendFL the
	\DIFdelbeginFL \DIFdelFL{area is}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{50\textsuperscript{th}}\DIFaddendFL , \DIFaddbeginFL \DIFaddFL{60\textsuperscript{th}, 70\textsuperscript{th},
	90\textsuperscript{th}, 95\textsuperscript{th}, and 99\textsuperscript{th}
	percentile of }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{more }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{of }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{active learner outperforms }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{pairwise L1-distance between }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{passive learner}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{data points}\DIFaddendFL . \DIFdelbeginFL \DIFdelFL{The top dotted line
	indicates }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{For
	}\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{maximum performance achieved by }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{glass dataset, changing value of $\gamma$ has minimal effect on the
	results, while for the ionosphere dataset, }\DIFaddendFL using the \DIFdelbeginFL \DIFdelFL{entire labeled
	pool as training data}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{90\textsuperscript{th}
	percentile and above seems to work well}\DIFaddendFL .}
	\DIFdelbeginFL %DIFDELCMD < \label{fig:strength}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \label{fig:gamma}
\DIFaddendFL \end{figure}

\DIFdelbegin \section{\DIFdel{Results and Discussion}}%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
%DIFDELCMD < \label{sec:discuss}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{figure}[p]
	\centering
	\includegraphics[width=0.7\textwidth]{Fig5}
	\caption[Effect of the parameters on Thompson sampling]{\DIFaddFL{Effect of the
	initial values of the parameters in }\textsc{\DIFaddFL{thompson}}\DIFaddFL{. We test 16
...
\DIFdelbegin \DIFdel{Figure}\DIFdelend \DIFaddbegin \DIFadd{Figures}\DIFaddend ~\ref{fig:strengths-small} and~\ref{fig:strengths-large} show the
strengths of all methods that we consider, while
\DIFdelbegin \DIFdel{Figure}\DIFdelend \DIFaddbegin \DIFadd{Figures}\DIFaddend ~\ref{fig:learning_curves-small} and~\ref{fig:learning_curves-large}
...
\DIFdelbegin \DIFdel{Figure}\DIFdelend \DIFaddbegin \DIFadd{Figures}\DIFaddend ~\ref{fig:strengths-small} and Figure~\ref{fig:learning_curves-small}.
...
\DIFdelbegin \DIFdel{We observe that with the small and medium-sized datasets, the effect of active
learning is minimal. The box plots for all of these datasets cross the
zero
line, which means that there is no statistical difference between the corresponding heuristic/combiner and passive learning . With the pima dataset ,
active
learning appears to perform better if we look at the raw accuracy score.
However from the MPBA score, where the class imbalance is taken into account,
the benefit starts to disappear.
It is only with the 4 large datasets
that
}\DIFdelend \DIFaddbegin \DIFadd{Active learning methods generally beat passive learning in four of the six
...
\DIFadd{The advantage of active learning becomes more apparent with the larger datasets
like magic, miniboone, pageblocks, and sdss. Here }\DIFaddend there is a visible gap
between the passive learning curve and the active learning curve for most
methods. \DIFaddbegin \DIFadd{For instance, using a simple heuristic such as }\textsc{\DIFadd{confidence}} \DIFadd{in
...
\textsc{w-entropy}) \DIFdelbegin \DIFdel{outperform passive learning only in
the pageblocks dataset (the
most imbalanced dataset, with 90\% of the data
being in the majority class)}\DIFdelend \DIFaddbegin \DIFadd{generally perform worse than the ones without the
weighting}\DIFaddend . \textsc{qbb-kl} performs \DIFdelbegin \DIFdel{slightly better in pageblocks and miniboone}\DIFdelend \DIFaddbegin \DIFadd{the best in pageblocks while it can barely
beat passive learning in other datasets}\DIFaddend . The remaining
heuristics---\textsc{confidence}, \textsc{margin}, \textsc{entropy}, and
\textsc{qbb-margin}---perform equally well in \DIFdelbegin \DIFdel{magic, miniboone, pageblocks, and sdss}\DIFdelend \DIFaddbegin \DIFadd{all datasets}\DIFaddend .
...
from \textsc{explore}, where we explore 100\% of the time \DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{(see Figure
...
heuristics }\textsc{\DIFadd{thompson}} \DIFadd{picks seems to correlate with the heuristic
performance. For example, as shown in Figure
\ref{fig:selection-thompson-params}, }\textsc{\DIFadd{passive}}
\DIFadd{and }\textsc{\DIFadd{qbb-kl}} \DIFadd{tend to get chosen less often than others in the ionosphere
dataset.
}\DIFaddend


\DIFdelbegin \subsection{\DIFdel{Discussion}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \begin{figure}[tbp]
	\centering
	\includegraphics[width=\textwidth]{Fig6}
	\caption[Policy strength]{\DIFaddFL{Boxplots of the accuracy and MPBA strength of the
...
\begin{figure}[tbp]
	...
	\caption[Selected learning curves]{\DIFaddFL{Selected accuracy and MPBA learning
	...
	show the learning curve for }\textsc{\DIFaddFL{passive}}\DIFaddFL{, }\textsc{\DIFaddFL{confidence}}\DIFaddFL{,
	}\textsc{\DIFaddFL{exp3++}}\DIFaddFL{, and }\textsc{\DIFaddFL{borda}}\DIFaddFL{. The learning curves are averaged over
...
\begin{figure}[tbp]
	...
	curves, we only show the learning curve for }\textsc{\DIFaddFL{passive}}\DIFaddFL{,
	}\textsc{\DIFaddFL{confidence}}\DIFaddFL{, }\textsc{\DIFaddFL{exp3++}}\DIFaddFL{, and }\textsc{\DIFaddFL{borda}}\DIFaddFL{. The learning
...
\begin{figure}[tbp]
	...
	\caption[Heuristic selection frequencies]{\DIFaddFL{Selection frequencies of
	heuristics in }\textsc{\DIFaddFL{thompson}} \DIFaddFL{and }\textsc{\DIFaddFL{exp3++}}\DIFaddFL{, with the large
	...
	selection frequencies are averaged over 10 trials. }\textsc{\DIFaddFL{thompson}} \DIFaddFL{favors
	certain heuristics more strongly than others. In contrast, }\textsc{\DIFaddFL{exp3++}}
	\DIFaddFL{favors uniform exploration more, sampling each heuristic with roughly equal
	weights. The plots for }\textsc{\DIFaddFL{ocucb}} \DIFaddFL{and }\textsc{\DIFaddFL{klucb}} \DIFaddFL{are not shown
	here, but they are similar to }\textsc{\DIFaddFL{exp3++}}\DIFaddFL{.}}
	\label{fig:selection}
\end{figure}
...
\begin{figure}[tbp]
	...
	selection]{\DIFaddFL{The effect of the initial values of the parameters in
	}\textsc{\DIFaddFL{thompson}} \DIFaddFL{on the heuristic selection frequencies. We test 16
	combinations of $\mu$, $\sigma^2$, and $\tau^2$ on the glass and ionosphere
	dataset. Which heuristics }\textsc{\DIFaddFL{thompson}} \DIFaddFL{picks seems to correlate with
	the heuristic performance. For example, in ionosphere, }\textsc{\DIFaddFL{passive}}
	\DIFaddFL{(the dotted purple line) and }\textsc{\DIFaddFL{qbb-kl}} \DIFaddFL{(the dashed dark blue line)
...
\begin{figure}[tbp]
	...
	\caption[Effect of the pool size]{\DIFaddFL{Effect of the pool size on the learning
...
\section{\DIFadd{Discussion}}\label{sec:discussion}

\DIFaddend The experimental results allow us to answer the following questions:

\begin{enumerate}
	\item \textbf{Can active learning beat passive learning?} Yes, active
	learning can perform much better than passive learning\DIFaddbegin \DIFadd{, especially }\DIFaddend when the
	unlabeled pool is large (e.g. sdss, miniboone, pageblock). When the
	\DIFdelbegin \DIFdel{dataset }\DIFdelend \DIFaddbegin \DIFadd{unlabeled pool }\DIFaddend is small, \DIFdelbegin \DIFdel{we do not actually need active learning , since we can manually label everything }\DIFdelend \DIFaddbegin \DIFadd{the effect of active learning becomes less
	...
	small, we can label everything so active learning is usually not needed}\DIFaddend .
	...
	example, \textsc{w-entropy} actually slows down the learning in the \DIFdelbegin \DIFdel{magic
	dataset}\DIFdelend \DIFaddbegin \DIFadd{many
	datasets}\DIFaddend . However, this only happens with certain heuristics, like those
	...
	\begin{enumerate}
		...
		\DIFdelbegin \DIFdel{require }\DIFdelend \DIFaddbegin \DIFadd{assumes }\DIFaddend that the reward follows a certain distribution (e.g. Gaussian).
		\DIFdelbegin \DIFdel{This can be hard to satisfy}\DIFdelend \DIFaddbegin \DIFadd{However, this assumption is unrealistic}\DIFaddend .
	\end{enumerate}
	...
	\begin{enumerate}
		...
		\DIFdelbegin \DIFdel{magntitudes}\DIFdelend \DIFaddbegin \DIFadd{magnitudes}\DIFaddend , which could cause a loss of information.
...
\DIFaddbegin \DIFadd{Our investigation has a few limitations. Firstly, we empirically compare
...
scores among the classes. As suggested by \mbox{%DIFAUXCMD
\cite{reyes18}}\hspace{0pt}%DIFAUXCMD
, we can use any
...
\DIFadd{Another limitation of our methods is that our active learning methods are
...
for example by using cluster analysis \mbox{%DIFAUXCMD
\citep{xu07} }\hspace{0pt}%DIFAUXCMD
or by maximizing the
diversity in a batch \mbox{%DIFAUXCMD
\citep{brinker03}}\hspace{0pt}%DIFAUXCMD
. How to aggregate suggestions from these
...
%DIFDELCMD < 	\centering
%DIFDELCMD < 	\begin{subfigure}[t]{\textwidth}
%DIFDELCMD <         \centering
%DIFDELCMD <         \includegraphics[width=\textwidth]{figures/strengths-accuracy-small}
%DIFDELCMD < 		%%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Accuracy score}}
		%DIFAUXCMD
%DIFDELCMD < \label{fig:strengths-accuracy-small}
%DIFDELCMD < 	\end{subfigure}
%DIFDELCMD < 	\begin{subfigure}[t]{\textwidth}
%DIFDELCMD <         \centering
%DIFDELCMD <         \includegraphics[width=\textwidth]{figures/strengths-mpba-small}
%DIFDELCMD < 		%%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{MPBA score}}
		%DIFAUXCMD
%DIFDELCMD < \label{fig:strengths-accuracy-large}
%DIFDELCMD <     \end{subfigure}
%DIFDELCMD < 	%%%
%DIFDELCMD < \caption[Policy strength]{%
{%DIFAUXCMD
\DIFdelFL{Boxplots of the accuracy and MPBA strength of the
	...
	%DIFAUXCMD
%DIFDELCMD < \label{fig:strengths-small}
...
%DIFDELCMD < 	\begin{subfigure}[t]{\textwidth}
%DIFDELCMD <         \centering
%DIFDELCMD <         \includegraphics[width=\textwidth]{figures/strengths-accuracy-large}
%DIFDELCMD < 		%%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Accuracy score}}
		%DIFAUXCMD
%DIFDELCMD < \label{fig:strengths-mpba-small}
%DIFDELCMD < 	\end{subfigure}
%DIFDELCMD < 	\begin{subfigure}[t]{\textwidth}
%DIFDELCMD <         \centering
%DIFDELCMD <         \includegraphics[width=\textwidth]{figures/strengths-mpba-large}
%DIFDELCMD < 		%%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{MPBA score}}
		%DIFAUXCMD
%DIFDELCMD < \label{fig:strengths-mpba-large}
%DIFDELCMD <     \end{subfigure}
%DIFDELCMD < 	%%%
%DIFDELCMD < \caption[Policy strength]{%
{%DIFAUXCMD
\DIFdelFL{Boxplots of the accuracy and MPBA strength of the
	...
	%DIFAUXCMD
%DIFDELCMD < \label{fig:strengths-large}
%DIFDELCMD < \end{figure}
%DIFDELCMD <
...
%DIFDELCMD < 	\begin{subfigure}[t]{\textwidth}
%DIFDELCMD <         \centering
%DIFDELCMD <         \includegraphics[width=\textwidth]{figures/learning_curves-accuracy-small}
%DIFDELCMD < 		%%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Accuracy learning curves}}
		%DIFAUXCMD
%DIFDELCMD < \label{fig:learning_curves-accuracy-small}
%DIFDELCMD < 	\end{subfigure}
%DIFDELCMD < 	\begin{subfigure}[t]{\textwidth}
%DIFDELCMD <         \centering
%DIFDELCMD <         \includegraphics[width=\textwidth]{figures/learning_curves-mpba-small}
%DIFDELCMD < 		%%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{MPBA learning curves}}
		%DIFAUXCMD
%DIFDELCMD < \label{fig:learning_curves-accuracy-large}
%DIFDELCMD <     \end{subfigure}
%DIFDELCMD < 	%%%
%DIFDELCMD < \caption[Selected learning curves]{%
{%DIFAUXCMD
\DIFdelFL{Selected accuracy and MPBA learning
	curves for the small datasets (glass, ionosphere, iris, sonar, wine, and
	wpbc). As it would get too cluttered to plot 17 learning curves, we only
	show the learning curve for }\textsc{\DIFdelFL{passive}}%DIFAUXCMD
\DIFdelFL{, }\textsc{\DIFdelFL{confidence}}%DIFAUXCMD
\DIFdelFL{,
	}\textsc{\DIFdelFL{exp3++}}%DIFAUXCMD
\DIFdelFL{, and }\textsc{\DIFdelFL{borda}}%DIFAUXCMD
\DIFdelFL{. The learning curves are averaged over
	10 trials.}}
	%DIFAUXCMD
%DIFDELCMD < \label{fig:learning_curves-small}
%DIFDELCMD < \end{figure}
%DIFDELCMD <
...
%DIFDELCMD < 	\begin{subfigure}[t]{\textwidth}
%DIFDELCMD <         \centering
%DIFDELCMD <         \includegraphics[width=\textwidth]{figures/learning_curves-accuracy-large}
%DIFDELCMD < 		%%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Accuracy learning curves}}
		%DIFAUXCMD
%DIFDELCMD < \label{fig:learning_curves-mpba-small}
%DIFDELCMD < 	\end{subfigure}
%DIFDELCMD < 	\begin{subfigure}[t]{\textwidth}
%DIFDELCMD <         \centering
%DIFDELCMD <         \includegraphics[width=\textwidth]{figures/learning_curves-mpba-large}
%DIFDELCMD < 		%%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{MPBA learning curves}}
		%DIFAUXCMD
%DIFDELCMD < \label{fig:learning_curves-mpba-large}
%DIFDELCMD <     \end{subfigure}
%DIFDELCMD < 	%%%
%DIFDELCMD < \caption[Selected learning curves]{%
{%DIFAUXCMD
\DIFdelFL{Selected accuracy and MPBA learning
	curves for the medium to large datasets (magic, miniboone, pageblocks,
	pima, sdss, and vehicle). As it would get too cluttered to plot 17 learning
	curves, we only show the learning curve for }\textsc{\DIFdelFL{passive}}%DIFAUXCMD
\DIFdelFL{,
	}\textsc{\DIFdelFL{confidence}}%DIFAUXCMD
\DIFdelFL{, }\textsc{\DIFdelFL{exp3++}}%DIFAUXCMD
\DIFdelFL{, and }\textsc{\DIFdelFL{borda}}%DIFAUXCMD
\DIFdelFL{. The learning
	curves are averaged over 10 trials.}}
	%DIFAUXCMD
%DIFDELCMD < \label{fig:learning_curves-large}
%DIFDELCMD < \end{figure}
%DIFDELCMD <
...
%DIFDELCMD < 	\begin{subfigure}[t]{\textwidth}
%DIFDELCMD <         \centering
%DIFDELCMD <         \includegraphics[width=\textwidth]{figures/selection-thompson-large}
%DIFDELCMD < 		%%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Thompson sampling}}
		%DIFAUXCMD
%DIFDELCMD < \label{fig:selection-thompson-large}
%DIFDELCMD < 	\end{subfigure}
%DIFDELCMD < 	\begin{subfigure}[t]{\textwidth}
%DIFDELCMD <         \centering
%DIFDELCMD <         \includegraphics[width=\textwidth]{figures/selection-exp++-large}
%DIFDELCMD < 		%%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{EXP3++ algorithm}}
		%DIFAUXCMD
%DIFDELCMD < \label{fig:selection-exp-large}
%DIFDELCMD <     \end{subfigure}
%DIFDELCMD < 	%%%
%DIFDELCMD < \caption[Heuristic selection frequencies]{%
{%DIFAUXCMD
\DIFdelFL{Selection frequencies of
	heuristics in }\textsc{\DIFdelFL{thompson}} %DIFAUXCMD
\DIFdelFL{and }\textsc{\DIFdelFL{exp3++}}%DIFAUXCMD
\DIFdelFL{, with the large
	datasets (magic, miniboone, pageblocks, pima, sdss, and vehicle). The plots
	show how often each of the heuristics gets selected over time. The
	selection frequencies are averaged over 10 trials. }\textsc{\DIFdelFL{thompson}} %DIFAUXCMD
\DIFdelFL{favors
...
\DIFdelFL{favors uniform exploration more, sampling each heuristic with roughly equal
	weights. The plots for }\textsc{\DIFdelFL{ocucb}} %DIFAUXCMD
\DIFdelFL{and }\textsc{\DIFdelFL{klucb}} %DIFAUXCMD
\DIFdelFL{are not shown
	here, but they are similar to }\textsc{\DIFdelFL{exp3++}}%DIFAUXCMD
\DIFdelFL{.}}
	%DIFAUXCMD
%DIFDELCMD < \label{fig:selection}
%DIFDELCMD < \end{figure}
%DIFDELCMD <
...
Repository}\footnote{%DIFDELCMD < \url{http://archive.ics.uci.edu/ml}%%%
}%DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdel{. In particular, the
vehicle
dataset}\footnote{%DIFDELCMD < \url{https://archive.ics.uci.edu/ml/datasets/Statlog+(Vehicle+Silhouettes)}%%%
}
%DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
...
%DIFDELCMD < %%%
\DIFdel{The SDSS dataset}\footnote{%DIFDELCMD < \url{http://dx.doi.org/10.5281/zenodo.58500}%%%
} %DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdel{was
...
distribution:
	\begin{align}
		(g_i \mid A_i) &\sim \DIFdelbegin %DIFDELCMD < & %%%
\DIFdelend \Bin(n_i, A_i)
	\end{align}
...
simply multiply the prior with the likelihood:
	\begin{align}
		f_{A_i \mid \bm{g}}(a)
		&\propto \DIFdelbegin %DIFDELCMD < & %%%
\DIFdelend f_{A_i}(a) \times f_{g_i \mid A_i}(g_i) \\
		&\propto \DIFdelbegin %DIFDELCMD < & %%%
\DIFdelend a^{1-1}(1-a)^{1-1} \times a^{g_i} (1 - a)^{n_i - g_i} \\
...
distribution, now with parameters
	\begin{align}
		(A_i \mid g_i) &\sim \DIFdelbegin %DIFDELCMD < & %%%
\DIFdelend \Beta(1 + g_i, 1 + n_i - g_i)
	\end{align}
