...
%DIF ADD combining-active-learners.tex      Tue Mar 13 10:28:59 2018
...
\DIFaddbegin \DIFadd{Rank aggregation methods have been used in the context of multi-task learning
for linguistic annotations \mbox{%DIFAUXCMD
\citep{reichart08}}\hspace{0pt}%DIFAUXCMD
, where we have one active learner
selecting examples to improve the performance of multiple tasks (e.g.
part-of-speech tagging and name entity recognition). }\DIFaddend However, as far as we
...
computationally inefficient. \DIFaddbegin \DIFadd{A more comprehensive treatment of these active
...
$\gamma$:
	...
	\DIFadd{\text{sim}(\bm{x}, \bm{x}^{(k)})
		}&\DIFadd{= \exp(\gamma ||\bm{x} - \bm{x}^{(k)}||^2)
}\end{align}
...
Out of the five heuristics discussed, how do we know which one is the \DIFdelbegin \DIFdel{most
optimal one}\DIFdelend \DIFaddbegin \DIFadd{best}\DIFaddend ?
...
maximize the gambler's lifetime rewards while minimizing the number of pulls.
...
produces probability estimates directly, rather than SVMs which only produce
unnormalized scores. }\DIFaddend \cite{hsu15} studied an improved version \DIFdelbegin \DIFdel{,
}\DIFdelend \DIFaddbegin \DIFadd{of EXP4, called
}\DIFaddend EXP4.P, \DIFdelbegin \DIFdel{along with }\DIFdelend \DIFaddbegin \DIFadd{and used }\DIFaddend importance weighting to estimate the \DIFdelbegin \DIFdel{rewards }\DIFdelend \DIFaddbegin \DIFadd{true classifier
...
However, being a bandit method, they require designing a reward schema. If the
...
Table~\ref{tab:rank} for an \DIFdelbegin \DIFdel{exmaple}\DIFdelend \DIFaddbegin \DIFadd{example}\DIFaddend .
...
With \DIFdelbegin \DIFdel{the least confidence, smallest margin, and highest entropy
heuristics, we also test a slight variant (}\DIFdelend \textsc{w-confidence}, \textsc{w-margin}, \DIFaddbegin \DIFadd{and }\DIFaddend \textsc{w-entropy}\DIFdelbegin \DIFdel{), in which we weight the score with the
...
the instance with feature vetor $\bm{x}$}\DIFdelend , \DIFdelbegin \DIFdel{$E$ is the size of the candidate pool
...
%DIFAUXCMD
\DIFdel{We }\DIFdelend \DIFaddbegin \DIFadd{we }\DIFaddend set
$\gamma$ in the \DIFdelbegin \DIFdel{experiments }\DIFdelend \DIFaddbegin \DIFadd{Gaussian kernel }\DIFaddend to be the inverse of the 90\textsuperscript{th}
percentile of all pairwise distances.
\DIFdelbegin \DIFdel{The information
...
obtained either by the higher of: 1) the performance achieved by using all the
labeled examples in the training set; and 2) the maximum value of the learning
curves of all the methods.}}\DIFaddend , and $m(h, t)$ is the performance achieved using
the first $t$ examples selected by heuristic $h$. We can think of the summation
as the area between the best possible performance line and the learning curve
of $h$. The better the heuristic is, the faster it would approach this maximum
line, and thus the smaller the area. Finally, so that we can compare the
performance across datasets, we normalize the measure with the area obtained
from using just passive learning. Refer to Figure~\ref{fig:strength} for a
visualization of the strength measure.
...
\begin{figure}[htbp]
	...
	indicates the maximum performance achieved\DIFdelbeginFL \DIFdelFL{by using the entire labeled
	pool as training data}\DIFdelendFL .}
	\label{fig:strength}
\end{figure}
