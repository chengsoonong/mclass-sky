...
Section~\ref{sec:heuristics} describes two family of algorithms in
...
application to active learning is described in Section~\ref{sec:expert}. To
compare the algorithms, we run them on 11 benchmark datasets from UCI and a
large dataset from the Sloan Sky Digital Sky Survey (SDSS). The experiment
setup and discussion

The key empirical finding is that combining suggestions, even with a very simple
aggregation rule like Borda count, leads to better performance, especially
in large datasets with unbalanced classes.
...
rule is an objective function $r$ that picks the most most informative example
based on the scores for labeling:
\begin{IEEEeqnarray*}{lCl}
	r : \mathbb{R}^{U} \rightarrow J_\Unlabelled
\end{IEEEeqnarray*}
where $J_\Unlabelled$ is the index set over the unlabelled instances. This can
be as simple as an argmax or argmin function, depending on how the scores are
interpreted.
...
There are also heuristics that require minimizing the variance or the
classifier certainty of the model \citep{schein07}, but we do not consider them
in this paper as they are computationally inefficient in practice.
...
\begin{table}[h]
	\caption {Summary of active learning heuristics used in our experiments,
			  ...
			  $\B$ is the set of $B$ hypotheses $\{h_1, h_2, ..., h_B\}$, $\Y$
	...
	\begin{tabular}{lll}
		...
		Highest Entropy &
			...
			\\[2ex]
		Smallest Margin &
			$\argmin_{x \in \Unlabelled} \left\{ \max_{y \in \Y} p(y | \bm{x}; h) -
            \max_{z \in \Y \setminus \{y\}} p(z | \bm{x}; h)  \right\}$
			\\[2ex]
		Smallest QBB Margin &
			$\argmin_{x \in \Unlabelled} \left\{ \max_{y \in \Y} p(y | \bm{x}; \B) -
...
$k(r_i)$ which converts the scores into a preference ordering:
	...
	\end{IEEEeqnarray*}
where $\sigma(J_\Unlabelled)$ is a permutation over the index set of
$\Unlabelled$. An aggregation function $c$ will
...
assume labels are available here. We initialize the classifier by labelling 10
