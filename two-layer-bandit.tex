...
as the reward. Compared to the raw accuracy score, this metric takes into
...
density weighting is proposed by~\cite{settles08} to discourage the acitve
learner from picking outliers. Although the class membership of outliers might
be uncertain, knowing their labels would probably not affect the classifer's
...
\end{IEEEeqnarray}
where $m$ is a chosen metric (e.g. accuracy rate, MPBA), $m(\max)$
is the best possible performance achieved from using all labeled data in the
...
the test set where the predicted label matches the true label. If a dataset had
a dominant class, then the accuracy score of instances within that class would
...
Figure~\ref{fig:strengths-accuracy} and~\ref{fig:strengths-mpba} show the
strengths of all methods that we consider, while
Figure~\ref{fig:learning_curves-accuracy} and~\ref{fig:learning_curves-mpba}
provide selected learning curves. Each figure contains two subfigures. The top
subfigure is for the 6 small datasets with fewer than 500 examples (glass,
ionosphere, iris, sonar, wine, and wpbc). The bottom subfigure is for the 2
medium-sized datasets (pima and vehicle) and the 4 large datasets (magic,
miniboone, pageblocks, and sdss).
...
\textsc{w-margin}, \textsc{w-entropy}) outperform passive learning only in
the pageblocks dataset (the most imbalanced dataset, with 90\% of the examples
...
\begin{enumerate}
	...
	dataset. However, this only happens with certain heuristics (e.g.
	when we use information density weighting).

	\item \textbf{What is the best single active learning heursitic?} Although
	all four heuristics---\textsc{confidence}, \textsc{margin},
	\textsc{entropy}, and \textsc{qbb-margin}---have a similar performance,
	\textsc{confidence} is perhaps the simpliest to compute and thus is a good
	default choice.

	\item \textbf{Which bandit algorithm should I use in practice to combine
	active leaners?} We do not recommend using any bandit algorithm to combine
	active leaners. There are two reasons for this. First, we do not have a
	...
	need to ensure that every training example and active learning suggestion
	have a non-zero probability of being selected in every iteration. Solving
	the second problem is more tricky. Is the increase in the classifer
	performance actually a good reward scheme? This type of reward is
	non-stationary (getting smaller after each step as learning saturates) and
	eventually the rewards actually go to zero.

	\item \textbf{Which rank aggregator should I use in practice to combine
	active leaners?} Given that there is no difference between the three rank
	...
	of the reward (we simply give all suggestions an equal weight when
	combining). Thus we neither need keep a keep a separate test set, nor do we
...
\begin{figure}[tbp]
	...
	\begin{subfigure}[t]{\textwidth}
		...
		\caption{Medium-sized and large datasets}
...
\begin{figure}[tbp]
    ...
    \end{subfigure}
	\caption[Policy strength]{Boxplots of MPBA strength of the 16 active
	learning strategies, relative to passive learning. The more positive the
	strength is, the better the heuristic/combiner is. Blue boxes represent
	individual heuristics; red boxes represent bandit alogrithms, and green
	boxes are for rank aggragation methods. A strategy that is above the zero
	line is better than passive learning. Each box contains 10 trials. The
	accuracy rate is a simple metric that simply counts up the number of
...
\begin{figure}[tbp]
	...
	As it would get too cluttered to plot 16 learning curves, we only show the
	MPBA learning curves for passive learning, the least confidence
	heuristic, the EXP3++ bandit policy, and the Borda aggregator. The
	learning curves are averaged over 10 trials.}
