...
From these scores, we obtain a preference ordering $R_i$, where the higher a
candidate is ranked corresponds to the one where the heuristic thinks it will
give us more information upon labeling. If we have $s$ heuristics, we end up
with a set of $s$ orderings called a profile, $\{ R_1, R_2, \ldots, R_s \}$.
...
we first compute $d(i, j)$, the number of heuristics that prefer candidate $i$
