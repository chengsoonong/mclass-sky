...
supervised machine learning algorithms, and the labeling process has become a
significant bottleneck. This paper explores the use of active learning
algorithms to identify informative examples for labeling, We empirically
compare several active learning heuristics on benchmark UCI datasets and a
large dataset from the astronomical domain. By considering each active learning
heuristic as an expert that gives recommendation on which example to label
next, we propose to combine them using bandit and rank aggregation algorithms.
Our results show that combining active learning suggestions improves over each
individual heuristic (including passive learning) and the computation is
...
informative candidate is given a rank of 1:
	\begin{IEEEeqnarray*}{lCl}
		k : \mathbb{R}^{|U|} \rightarrow \sigma(J_U)
	\end{IEEEeqnarray*}
