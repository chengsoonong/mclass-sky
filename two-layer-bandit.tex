...
classifiers, we propose to combine active learners with bandit algorithms and
...
problem. Many techniques such as AdaBoost (which averages predictions from a
set of models) and decision trees (which selects one model for making
predictions based on the input space) have shown to perform better than just
...
Section~\ref{sec:expert}. The use of bandit algorithms, which pick one
heuristic at each time step, have been studied before~\citep{baram04, hsu15}.
However, as far as we know, this is the first time that social choice theory is
used to rank and aggregate the suggestions.
...
setup and discussion are described in Section~\ref{sec:expt}. Our three main
findings are: active learning is better than passive learning, combining
active learners does not in general degrade the performance, and social choice
theory provides more practical algorithms than bandit theory since we do not
need to design a reward scheme.
