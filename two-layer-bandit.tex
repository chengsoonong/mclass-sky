...
benchmark datasets, and focus on its  application to photometric classification
...
in $\Unlabelled$ for querying. To quantify this, we need to come up with a rule
$r(\bm{x}; h)$ that can give each unlabelled example a score based only on
...
good heurstics. Five common ones are listed in Table \ref{tab:heuristics}.
...
\begin{figure}[tbp]
	\centering
	\includegraphics[width=\textwidth]{figures/active_learners}
	\caption[Some preliminary results.]{Some preliminary results. }
