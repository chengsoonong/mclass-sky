...
\begin{IEEEeqnarray}{lCl}
	\phi : \mathbb{R}^{U} \rightarrow J_\Unlabelled
\end{IEEEeqnarray}
...
\equiv (\phi \circ s)(\bm{x}; h)$, is the active leaner.
...
\begin{IEEEeqnarray}{lCl}
	r_{LC}(\bm{x}; h) &=& \argmax_{x \in \Unlabelled}
...
\begin{IEEEeqnarray}{lCl}
	r_{HE}(\bm{x}; h) &=& \argmin_{x \in \Unlabelled}
...
\begin{IEEEeqnarray}{lCl}
	r_{SM}(\bm{x}; h) &=& \argmin_{x \in \Unlabelled}
...
\begin{IEEEeqnarray}{lCl}
	r_{QBBM}(\bm{x}; h) &=& \argmin_{x \in \Unlabelled}
...
\begin{IEEEeqnarray}{lCl}
	r_{QBBKL}(\bm{x}; h) &=& \argmax_{x \in \Unlabelled}
	\left\{ \dfrac{1}{B}
...
up with the prediction is not important. At every step $t$, however, we have
the ability to ask each expert for a prediction, either of the next most
...
$\sigma^2$, the chosen heuristic would be
    ...
    \end{IEEEeqnarray}
Algorithm~\ref{alg:oc-ucb} and~\ref{alg:kl-ucb} summarize these two OCB
approaches.
...
each heuristic in advance. Like Thompson sampling, the algorithm samples from a
probability distribution at each step to pick a heursitic. Here however, we
construct the distribution with exponential weighting (hence the name EXP3,
which stands for exponential-weight algorithm for exploration and
...
each heuristic follows an unknown reward distribution) and adversarial regime.
...
which converts the scores into a preference ordering:
	\begin{IEEEeqnarray}{lCl}
...
arithmetic mean. An alternative approach is to use the geometric mean, where
instead of adding up the points, we multiply them~\citep{bedo14}.
...
\begin{table}[htbp]
	...
	\begin{subtable}{\linewidth}
		...
		the strongest path strengths $p(x_i, x_j)$ between all pairs of
		candidates. Candidate B is the winner since $p(B, A) > p(A, B)$, $p(B,
		C) > p(C, B)$, and $p(B, D) > p(D, B)$.}
	\end{subtable}
\end{table}
...
labeling 10 random samples and using them as the the initial training set. In
each step, we feed 300 randomly selected candidates from the unlabeled pool
into the algorith, i.e. $E = 300$. We use logistic regression with a Gaussian
kernel approximation and an L2 loss. In the binary case, the loss function is
\begin{IEEEeqnarray}{lCl}
	L &=& \dfrac{1}{2} \bm{w}^T \bm{w} + C \sum_{i = 1}^n
	      \ln\Big(1 + \exp(-y_i(f(\bm{x}_i)^T w))\Big)
\end{IEEEeqnarray}
where $\bm{x_i}$ is the feature vector of the $i$th example, $y_i \in \{0, 1\}$
is the label of $\bm{x_i}$, and $n$ is the training size. The first term of the
...
100-dimensional feature vector $f(\bm{x_i})$. In the multiclass case, we use the
...
For the bandit algorithms, the mean posterior balanced accuracy (MPBA) is used
as the reward. Compared to a simple accuracy rate, this measure takes into
account the class imbalance. Intuitively, we calculate the recall rate in each
class and then take the average, giving each class an equal weight. Refer to
Appendix A for the derivation of the MPBA.
...
Table~\ref{tab:abbre} for the list of abbreviations associated with the
methods.
\begin{table}[htbp]
...
\end{IEEEeqnarray}
We set $\gamma$ in the experiments to be the inverse of the 90th percentile of
all pairwise distances. The information density weighting is proposed
by~\cite{settles08} to discourage the acitve learner from picking outliers.
Although the class membership of outliers might be uncertain, knowing their
labels would probably not affect the classifer's performance on the data as a
whole.
...
\end{IEEEeqnarray}
where $m$ is a chosen metric (e.g. accuracy rate, F1-score), $m(\max)$ is the
best possible performance achieved from using all labeled data in the training
set, and $m(h, t)$ is the performance achieved using the first $t$ examples
selected by heuristic $h$. The summation can then be thought of as the area
between the best possible performance line and the learning curve of $h$. The
better the heuristic is, the faster it would approach this maximum line, and
thus the smaller the area. Finally, so that we can compare the performance
...
\begin{figure}[htbp]
	...
	\caption[Strength Measure]{An example of the strength measure. It is
	proportional to the shaded area between the (red) passive learning curve
	and the (blue) active learning curve. The bigger the area is, the more
	the active leaner outperforms the passive leaner. The top dotted line
	indicates the maximum performance (by using the entire lableled pool
	as training data).}
	\label{fig:strength}
\end{figure}
...
Figure~\ref{fig:strengths-accuracy} and~\ref{fig:strengths-f1} show the
strengths of all methods that we consider, while
Figure~\ref{fig:learning_curves-accuracy} and~\ref{fig:learning_curves-f1}
provide selected learning curves. Each figure contains two subfigures,
one for the 6 small datasets with fewer than 500 examples (glass, ionosphere,
iris, sonar, wine, and wpbc), and one for the 6 large datasets (magic,
miniboone, pageblocks, pima, sdss, and vehicle).

We observe that with the small datasets, the effect of active learning is
minimal. All the box plots in Figure \ref{fig:strengths-accuracy-small}
and \ref{fig:strengths-f1-small} cross the zero line, which means that there
is no statistical difference between the corresponding heuristic/combiner
and passive learning.

We start to benefit from active learning with the large datasets.

For small datasets with less than 500 examples (Figure
\ref{fig:strengths-accuracy-small}, \ref{fig:strengths-f1-small},
\ref{fig:learning_curves-accuracy-small}, and
\ref{fig:learning_curves-f1-small}), the effect of active learning is minimal.
All of the box plots in


With the exception of the wpbc
...
\begin{figure}[tbp]
    ...
    \end{subfigure}
	\caption[Policy strength]{Boxplots of accuracy strength of the 16 active
	learning strategies, relative to passive learning. The more positive the
	strength is, the better the heuristic/combiner is. Blue boxes represent
	individual heuristics; red boxes represent bandit alogrithms, and green
	boxes are for rank aggragation methods. A strategy that is above the zero
	line is better than passive learning. Each box contains 10 trials.
	The accuracy rate is a simple metric that simply counts up the number
...
\begin{figure}[tbp]
	...
	\begin{subfigure}[t]{\textwidth}
		...
		\caption{Small datasets}
		\label{fig:strengths-f1-small}
	\end{subfigure}
	\begin{subfigure}[t]{\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/strengths-f1-large}
		\caption{Large datasets}
		\label{fig:strengths-f1-large}
    \end{subfigure}
	\caption[Policy strength]{Boxplots of F1 strength of the 16 active learning
	strategies, relative to passive learning. The more positive the strength
	is, the better the heuristic/combiner is. Blue boxes represent individual
	heuristics; red boxes represent bandit alogrithms, and green boxes are for
	rank aggragation methods. A strategy that is above the zero line is better
	than passive learning. Each box contains 10 trials. The accuracy rate is a
	simple metric that simply counts up the number of correct predictions. The
	F1 score, being the weighted average of the recall and precision, gives an
...
\begin{figure}[tbp]
	...
	accuracy rate learning curves for passive learning, the least confidence
	heuristic, the EXP3++ bandit policy, and the Borda aggregator. The
	learning curves are averaged over 10 trials.}
...
\begin{figure}[tbp]
	...
	\begin{subfigure}[t]{\textwidth}
		...
		\caption{Small datasets}
		\label{fig:learning_curves-f1-small}
	\end{subfigure}
	\begin{subfigure}[t]{\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/learning_curves-f1-large}
		\caption{Large datasets}
		\label{fig:learning_curves-f1-large}
    \end{subfigure}
	\caption[Selected learning curves]{Selected F1-score learning curves.
	As it would get too cluttered to plot 16 learning curves, we only show the
	F1-score learning curves for passive learning, the least confidence
	heuristic, the EXP3++ bandit policy, and the Borda aggregator. The
	learning curves are averaged over 10 trials.}
