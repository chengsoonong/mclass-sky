...
that solves the trade-off from a Bayesian perspective. We also
...
combine the rankings. This leads us to Social Choice Theory. Originally developed
by political scientists like Condorcet, the theory is concerned with how we
aggregate preferences of a group of people to determine, for example, the winner
of an election. It has the nice property that everyone (or every suggestion in our context)
has a voice.

Formally, we first use heuristic $r_i$ to assign a score to every candidate. From these
score, we obtain a preference ordering $R_i$. Given any two candidates $x$ and $y$
